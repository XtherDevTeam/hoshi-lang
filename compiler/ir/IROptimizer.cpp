...
namespace yoi {
    IRFunctionOptimizer::SimulationStack::Item::ContributedInstructionSet::ContributedInstructionSet(yoi::indexT codeBlockIndex,
        ...
        : codeBlockIndex(codeBlockIndex), instructions(instructions) {}

    IRFunctionOptimizer::SimulationStack::Item::ContributedInstructionSet::ContributedInstructionSet(yoi::indexT codeBlockIndex,
                                                                                                     const std::set<yoi::indexT> &instructions,
                                                                                                     bool optimizable)
        : codeBlockIndex(codeBlockIndex), instructions(instructions), optimizable(optimizable) {}

    IRFunctionOptimizer::SimulationStack::Item::ContributedInstructionSet &
    IRFunctionOptimizer::SimulationStack::Item::ContributedInstructionSet::insert(yoi::indexT index) {
    ...
    IRFunctionOptimizer::SimulationStack::Item::ContributedInstructionSet::Iterator::Iterator(const std::set<yoi::indexT> &set) : it(set.begin()) {}

    IRFunctionOptimizer::SimulationStack::Item::ContributedInstructionSet::Iterator::Iterator(std::set<yoi::indexT>::const_iterator it) : it(it) {}

    bool IRFunctionOptimizer::SimulationStack::Item::ContributedInstructionSet::Iterator::operator!=(const Iterator &other) const {
        return it != other.it;
    }
    ...
    IRFunctionOptimizer::SimulationStack::Item::ContributedInstructionSet::Iterator::operator++() {
        ++it;
        return *this;
    }

    IRFunctionOptimizer::SimulationStack::Item::ContributedInstructionSet::Iterator
    IRFunctionOptimizer::SimulationStack::Item::ContributedInstructionSet::begin() const {
        return Iterator(instructions.begin());
    }

    IRFunctionOptimizer::SimulationStack::Item::ContributedInstructionSet::Iterator
    IRFunctionOptimizer::SimulationStack::Item::ContributedInstructionSet::end() const {
    ...
    IRFunctionOptimizer::SimulationStack::Item::PossibleValue::PossibleValue(yoi::indexT stringConstIndex) : stringConstIndex(stringConstIndex) {}
    ...
    yoi::indexT IRFunctionOptimizer::reduce(const SimulationStack::Item::ContributedInstructionSet &contributedInstructions,
        ...
        for (auto i : contributedInstructions) {
            targetFunction->codeBlock[contributedInstructions.codeBlockIndex]->getIRArray()[i] = {
                IR::Opcode::nop, {}, targetFunction->codeBlock[contributedInstructions.codeBlockIndex]->getIRArray()[i].debugInfo};
        ...
        if (a.hasPossibleValue && b.hasPossibleValue) {
            switch (a.type->type) {
                ...
                case IRValueType::valueType::characterObject: {
                    return {a.type, true, char{static_cast<char>(a.possibleValue.charValue + b.possibleValue.charValue)}, {}};
                }
                default:
    ...
    IRFunctionOptimizer::SimulationStack::Item IRFunctionOptimizer::negate(const IRFunctionOptimizer::SimulationStack::Item &a) {
        if (a.hasPossibleValue) {
            switch (a.type->type) {
                ...
                case IRValueType::valueType::characterObject: {
                    return {a.type, true, char{static_cast<char>(-a.possibleValue.charValue)}, {}, {}};
                }
                default:
    ...
    IRFunctionOptimizer::SimulationStack::Item IRFunctionOptimizer::bitwiseShiftLeft(const IRFunctionOptimizer::SimulationStack::Item &a,
        ...
        if (a.hasPossibleValue && b.hasPossibleValue) {
            switch (a.type->type) {
                ...
                case IRValueType::valueType::characterObject: {
                    return {a.type, true, char{static_cast<char>(a.possibleValue.charValue << b.possibleValue.intValue)}, {}, {}};
                }
                default:
    ...
    yoi::indexT IRFunctionOptimizer::generatePushOp(const SimulationStack::Item &item, yoi::indexT index) {
        ...
        switch (item.type->type) {
            case IRValueType::valueType::integerObject:
                IRArr.insert(IRArr.begin() + index + 1,
                             IR{IR::Opcode::push_integer,
                                {IROperand{IROperand::operandType::integer, {item.possibleValue.intValue}}},
                                IRArr[index].debugInfo});
                break;
            case IRValueType::valueType::unsignedObject:
                IRArr.insert(IRArr.begin() + index + 1,
                             IR{IR::Opcode::push_unsigned,
                                {{IROperand::operandType::unsignedInt, IROperand::operandValue{item.possibleValue.unsignedValue}}},
                                IRArr[index].debugInfo});
                break;
            case IRValueType::valueType::shortObject:
                IRArr.insert(IRArr.begin() + index + 1,
                             IR{IR::Opcode::push_short,
                                {{IROperand::operandType::shortInt, IROperand::operandValue{static_cast<int64_t>(item.possibleValue.shortValue)}}},
                                IRArr[index].debugInfo});
                break;
            case IRValueType::valueType::decimalObject:
                IRArr.insert(IRArr.begin() + index + 1,
                             IR{IR::Opcode::push_decimal,
                                {IROperand{IROperand::operandType::decimal, {item.possibleValue.deciValue}}},
                                IRArr[index].debugInfo});
                break;
            case IRValueType::valueType::booleanObject:
                IRArr.insert(IRArr.begin() + index + 1,
                             IR{IR::Opcode::push_boolean,
                                {{IROperand::operandType::boolean, IROperand::operandValue{item.possibleValue.boolValue}}},
                                IRArr[index].debugInfo});
                break;
            case IRValueType::valueType::stringObject:
                IRArr.insert(IRArr.begin() + index + 1,
                             IR{IR::Opcode::push_string,
                                {{IROperand::operandType::index, IROperand::operandValue{irModule->identifier}},
                                 {IROperand::operandType::stringLiteral, IROperand::operandValue{item.possibleValue.stringConstIndex}}},
                                IRArr[index].debugInfo});
                break;
            case IRValueType::valueType::characterObject:
                IRArr.insert(IRArr.begin() + index + 1,
                             IR{IR::Opcode::push_character,
                                {{IROperand::operandType::character, IROperand::operandValue{static_cast<int64_t>(item.possibleValue.charValue)}}},
                                IRArr[index].debugInfo});
                break;
            default:
    ...
    IRFunctionOptimizer::IRFunctionOptimizer(const std::shared_ptr<compilerContext> &compilerCtx,
        ...
        : compilerCtx(compilerCtx), irModule(irModule), targetFunction(nullptr), globalAnalysisResults(globalResults) {}
    ...
    IRFunctionOptimizer::SimulationStack::Item IRFunctionOptimizer::lessThan(const SimulationStack::Item &item, const SimulationStack::Item &right) {
    ...
    IRFunctionOptimizer::SimulationStack::Item IRFunctionOptimizer::equal(const SimulationStack::Item &item, const SimulationStack::Item &right) {
        if (item.hasPossibleValue && right.hasPossibleValue) {
            switch (item.type->type) {
                case IRValueType::valueType::integerObject: {
                    return {compilerCtx->getBoolObjectType(), true, {item.possibleValue.intValue == right.possibleValue.intValue}, {}};
    ...
    IRFunctionOptimizer::SimulationStack::Item IRFunctionOptimizer::notEqual(const SimulationStack::Item &item, const SimulationStack::Item &right) {
        if (item.hasPossibleValue && right.hasPossibleValue) {
            switch (item.type->type) {
                case IRValueType::valueType::integerObject: {
                    return {compilerCtx->getBoolObjectType(), true, {item.possibleValue.intValue != right.possibleValue.intValue}, {}};
    ...
    IRFunctionOptimizer &IRFunctionOptimizer::reduceRedundantConstantExpr() {
        for (yoi::indexT insIndex = 0; insIndex < targetFunction->codeBlock[currentCodeBlockIndex]->getIRArray().size(); ++insIndex) {
            ...
            // std::cout << currentCodeBlockIndex << " " << insIndex << " " <<  wstring2string(ins.to_string()) <<
            // std::endl;
            switch (ins.opcode) {
                ...
                case IR::Opcode::push_unsigned: {
                    simulationStack.push(compilerCtx->getUnsignedObjectType(), {currentCodeBlockIndex, {insIndex}}, ins.operands[0].value.unsignedV);
                    break;
                }
                ...
                case IR::Opcode::push_character: {
                    simulationStack.push(
                        compilerCtx->getCharObjectType(), {currentCodeBlockIndex, {insIndex}}, static_cast<char>(ins.operands[0].value.character));
                    break;
                }
                ...
                case IR::Opcode::add: {
                    ...
                    if (result.hasPossibleValue) {
                        ...
                        simulationStack.push(
                            result.type,
                            left.contributedInstructions + right.contributedInstructions +
                                SimulationStack::Item::ContributedInstructionSet{currentCodeBlockIndex, std::set{yoi::indexT{insIndex}}});
                    }
                    break;
                }
                case IR::Opcode::sub: {
                    ...
                    // std::cout << "simulate sub " << right.hasPossibleValue << " " << left.hasPossibleValue << " " <<
                    // result.hasPossibleValue << std::endl;
                    if (result.hasPossibleValue) {
                        insIndex = reduce(left.contributedInstructions, insIndex);
                        insIndex = reduce(right.contributedInstructions, insIndex);
                ...
                case IR::Opcode::mul: {
                    ...
                    // std::cout << "simulate mul " << right.hasPossibleValue << " " << left.hasPossibleValue << " " <<
                    // result.hasPossibleValue << std::endl;
                    if (result.hasPossibleValue) {
                        insIndex = reduce(left.contributedInstructions, insIndex);
                        insIndex = reduce(right.contributedInstructions, insIndex);
                ...
                case IR::Opcode::negate: {
                    ...
                    if (result.hasPossibleValue) {
                        ...
                        // lost information, push back
                        simulationStack.push(result.type,
                                             value.contributedInstructions + SimulationStack::Item::ContributedInstructionSet{
                                                                                 currentCodeBlockIndex, std::set{yoi::indexT{insIndex}}});
                    }
                    break;
                ...
                case IR::Opcode::left_shift: {
                    ...
                    if (result.hasPossibleValue) {
                        ...
                        simulationStack.push(
                            result.type,
                            value.contributedInstructions + shift.contributedInstructions +
                                SimulationStack::Item::ContributedInstructionSet{currentCodeBlockIndex, std::set{yoi::indexT{insIndex}}});
                    }
                    break;
                }
                ...
                case IR::Opcode::load_local: {
                    if (auto it = variablesExtraInfo.find(ins.operands[0].value.symbolIndex); it != variablesExtraInfo.end()) {
                        ...
                        if (it->second.hasPossibleValue && it->second.possibleValue.contributedInstructions.codeBlockIndex == currentCodeBlockIndex) {
                            ...
                            simulationStack.push(it->second.possibleValue.type,
                        ...
                        // well there does be a possibility that the variable will be not initialized, and this is
                        // parameter passing in this case, we can't optimize it
                        auto type = targetFunction->getVariableTable().get(ins.operands[0].value.symbolIndex);
                        simulationStack.push(type, {currentCodeBlockIndex, {insIndex}});
                ...
                case IR::Opcode::store_local: {
                    ...
                    // if this is the twice or more times, we can ignore the previous store command and use the new
                    // value directly
                    ...
                    simulationStack.pop();
                    if (auto it = variablesExtraInfo.find(ins.operands[0].value.symbolIndex);
                        it != variablesExtraInfo.end() && it->second.hasPossibleValue && !it->second.isReadAfterStore &&
                        it->second.possibleValue.contributedInstructions.codeBlockIndex == currentCodeBlockIndex) {
                ...
                case IR::Opcode::store_global: {
                    ...
                    if (*definitionType != *value.type && value.type->type == IRValueType::valueType::null &&
                        value.type->type == IRValueType::valueType::pointerObject) {
                        // type mismatch, panic
                ...
                case IR::Opcode::store_member: {
                    ...
                    if (*memberDef != *value.type && value.type->type != IRValueType::valueType::pointerObject &&
                        value.type->type != IRValueType::valueType::null) {
                        // type mismatch, panic
                ...
                case IR::Opcode::invoke_virtual: {
                    ...
                    auto returnType = compilerCtx->getImportedModule(ins.operands[0].value.symbolIndex)
                ...
                case IR::Opcode::new_struct: {
                    ...
                    simulationStack.push(
                        managedPtr(IRValueType{IRValueType::valueType::structObject, moduleIndex, ins.operands[1].value.symbolIndex}),
                        {currentCodeBlockIndex, {insIndex}, false});
                    break;
                }
                case IR::Opcode::construct_interface_impl: {
                    auto moduleIndex = ins.operands[0].value.symbolIndex;
                    auto interfaceImplDef =
                        compilerCtx->getImportedModule(moduleIndex)->interfaceImplementationTable[ins.operands[1].value.symbolIndex];
                    auto returnType = managedPtr(IRValueType{IRValueType::valueType::interfaceObject,
                                                             interfaceImplDef->implInterfaceIndex.first,
                                                             interfaceImplDef->implInterfaceIndex.second});
                    simulationStack.pop();
                    simulationStack.push(returnType, {currentCodeBlockIndex, {insIndex}, false});
                    break;
                }
                case IR::Opcode::jump_if_true: {
                    ...
                    if (condition.hasPossibleValue) {
                        if (condition.possibleValue.boolValue) {
                            ...
                            insIndex -= 1; // reduce the index by 1, to make sure we don't skip the next instruction,
                                           // since we have reduced the instruction before
                        }
                    } else {
                ...
                case yoi::IR::Opcode::new_array_interface: {
                    ...
                    auto baseType = managedPtr(IRValueType{ins.opcode == yoi::IR::Opcode::new_array_struct ? IRValueType::valueType::structObject
                ...
                case IR::Opcode::load_element: {
                    ...
                    simulationStack.pop();
                    simulationStack.pop();
                    simulationStack.push(managedPtr(array.type->getElementType()),
                    ...
                    switch (ins.opcode) {
                        ...
                        case IR::Opcode::new_dynamic_array_interface:
                            baseType = managedPtr(IRValueType{
                                IRValueType::valueType::interfaceObject, ins.operands[0].value.symbolIndex, ins.operands[1].value.symbolIndex});
                            break;
                        case IR::Opcode::new_dynamic_array_struct:
                            baseType = managedPtr(IRValueType{
    ...
    IRFunctionOptimizer &IRFunctionOptimizer::reduceRedundantCodeAfterRet() {
        auto it = std::find_if(targetFunction->codeBlock[currentCodeBlockIndex]->getIRArray().begin(),
                               targetFunction->codeBlock[currentCodeBlockIndex]->getIRArray().end(),
        ...
        if (it != targetFunction->codeBlock[currentCodeBlockIndex]->getIRArray().end()) {
            ...
            targetFunction->codeBlock[currentCodeBlockIndex]->getIRArray().erase(
                it + 1, targetFunction->codeBlock[currentCodeBlockIndex]->getIRArray().end());
        }
        return *this;
    }
                        ...
                        if (targetFunction->returnType->type == IRValueType::valueType::none) {
                            ...
                            targetBlock->getIRArray().push_back(
                                IR{IR::Opcode::load_local,
                                   {{IROperand::operandType::localVar, IROperand::operandValue{static_cast<yoi::indexT>(0)}}},
                                   targetFunction->debugInfo});
                            targetBlock->getIRArray().push_back(IR{IR::Opcode::ret, {}, targetFunction->debugInfo});
                        } else {
                            panic(targetFunction->debugInfo.line,
                                  targetFunction->debugInfo.column,
                                  "IROptimizer::controlFlowOptimization(): function " + wstring2string(targetFunction->name) +
                                      " has no return instruction in out block");
                        }
                    }
                    continue;
                }
                if (targetBlock->getIRArray().back().opcode != IR::Opcode::ret && targetBlock->getIRArray().back().opcode != IR::Opcode::ret_none) {
                    ...
                    if (targetFunction->returnType->type == IRValueType::valueType::none) {
                        ...
                        targetBlock->getIRArray().push_back(
                            IR{IR::Opcode::load_local,
                               {{IROperand::operandType::localVar, IROperand::operandValue{static_cast<yoi::indexT>(0)}}},
                               targetBlock->getIRArray().back().debugInfo});
            ...
            if (!targetFunction->codeBlock[i]->getIRArray().empty()) {
                ...
                bool isTerminator =
                    (lastIns.opcode == IR::Opcode::jump || lastIns.opcode == IR::Opcode::jump_if_false ||
                     lastIns.opcode == IR::Opcode::jump_if_true || lastIns.opcode == IR::Opcode::ret || lastIns.opcode == IR::Opcode::ret_none);
    ...
    bool AnalysisState::operator!=(const AnalysisState &other) const {
        ...
        for (const auto &[varIdx, info1] : variableStates) {
            ...
            if ((info1.possibleValue.type && info2.possibleValue.type) &&
                (*info1.possibleValue.type != *info2.possibleValue.type ||
                 info1.possibleValue.type->attributes != info2.possibleValue.type->attributes)) {
                return true;
            }
        }
    ...
    AnalysisState mergeStates(const AnalysisState &s1, const AnalysisState &s2) {
        ...
        for (size_t i = 0; i < s1.stack.items.size(); ++i) {
            ...
            auto mergedItem =
                IRFunctionOptimizer::SimulationStack::Item{item1.type, false, {}, {}, item1.contributedInstructions + item2.contributedInstructions};

            if (item1.hasPossibleValue && item2.hasPossibleValue) {
                if (item1.type->type == item2.type->type && item1.type->isBasicType() &&
    ...
    bool IRFunctionOptimizer::performNullableCheck() {
        ...
        for (indexT blockIdx = 0; blockIdx < targetFunction->codeBlock.size(); ++blockIdx) {
            if (targetFunction->codeBlock[blockIdx]->getIRArray().empty())
                continue;
        ...
        for (const auto &[varIndex, varType] : targetFunction->variableTable.getReversedVariableNameMap()) {
            ...
            for (const auto &[blockIndex, outState] : blockOutStates) {
                if (outState.variableStates.count(varIndex) &&
                    outState.variableStates.at(varIndex).possibleValue.type->hasAttribute(IRValueType::ValueAttr::Nullable)) {
                    isNullable = true;
                    break;
                }
            }
            ...
            switch (ins.opcode) {
                ...
                case IR::Opcode::invoke: {
                    ...
                    for (size_t i = 0; i < func->argumentTypes.size(); ++i) {
                        if (globalAnalysisResults.count(calleeId)) {
                            auto simType = simulationStack.peek(0);
                            auto paramIndex = func->argumentTypes.size() - i - 1;

                            if (simType.type->hasAttribute(IRValueType::ValueAttr::Nullable) &&
                                !globalAnalysisResults.at(calleeId).paramStates.empty()) {
                                if (globalAnalysisResults.at(calleeId).paramStates[paramIndex] == FunctionAnalysisInfo::ParameterState::Raw) {
                                    warning(ins.debugInfo.line,
                                            ins.debugInfo.column,
                ...
                case IR::Opcode::invoke_virtual: {
                    ...
                    auto returnType = managedPtr(*compilerCtx->getImportedModule(ins.operands[0].value.symbolIndex)
                                                      ->interfaceTable[ins.operands[1].value.symbolIndex]
                                                      ->methodMap[ins.operands[2].value.symbolIndex]
                                                      ->returnType);
                    simulationStack.pop();
                ...
                case IR::Opcode::invoke_imported: {
                    ...
                    if (function->hasAttribute(IRFunctionDefinition::FunctionAttrs::NoFFI) &&
                        !function->hasAttribute(IRFunctionDefinition::FunctionAttrs::Intrinsic))
                        returnType->addAttribute(IRValueType::ValueAttr::Nullable); // Rule 3
                ...
                case IR::Opcode::left_shift:
                case IR::Opcode::right_shift: {
                    auto r = simulationStack.peek(0);
                    simulationStack.pop();
                    auto l = simulationStack.peek(0);
                    simulationStack.pop();
                ...
                case IR::Opcode::negate:
                case IR::Opcode::bitwise_not: {
                    auto val = simulationStack.peek(0);
                    simulationStack.pop();
                ...
                case IR::Opcode::basic_cast_char: {
                    ...
                    if (ins.opcode == IR::Opcode::basic_cast_int)
                        targetType = compilerCtx->getIntObjectType();
                    else if (ins.opcode == IR::Opcode::basic_cast_deci)
                        targetType = compilerCtx->getDeciObjectType();
                    else if (ins.opcode == IR::Opcode::basic_cast_bool)
                        targetType = compilerCtx->getBoolObjectType();
                    else
                        targetType = compilerCtx->getCharObjectType();
                ...
                case IR::Opcode::store_global: {
                    if (!simulationStack.items.empty())
                        simulationStack.pop();
                    break;
                }
                default:
    ...
    AnalysisState IRFunctionOptimizer::mergeStatesForNullable(const AnalysisState &s1, const AnalysisState &s2) {
        if (s1.variableStates.empty())
            return s2;
        if (s2.variableStates.empty())
            return s1;
        ...
        for (size_t i = 0; i < s1.stack.items.size(); ++i) {
            ...
            auto mergedItem = IRFunctionOptimizer::SimulationStack::Item{mergedType, false, {}, {}};
            mergedState.stack.push(mergedItem);
        }
    ...
    bool IRFunctionOptimizer::performRawCheck() {
        ...
        for (const auto &[varIndex, varType] : targetFunction->variableTable.getReversedVariableNameMap()) {
            ...
            if (blockOutStates.empty())
                isRaw = false; // No reachable blocks

            for (const auto &[blockIndex, outState] : blockOutStates) {
                if (outState.variableStates.count(varIndex) &&
                    !outState.variableStates.at(varIndex).possibleValue.type->hasAttribute(IRValueType::ValueAttr::Raw)) {
                    isRaw = false;
                    break;
                }
            }
            ...
            switch (ins.opcode) {
                ...
                case IR::Opcode::basic_cast_short: {
                    ...
                    if (ins.opcode == IR::Opcode::basic_cast_int)
                        ...
                        targetType = compilerCtx->getBoolObjectType();
                    else if (ins.opcode == IR::Opcode::basic_cast_char)
                        targetType = compilerCtx->getCharObjectType();
                    else if (ins.opcode == IR::Opcode::basic_cast_unsigned)
                        targetType = compilerCtx->getUnsignedObjectType();
                    else if (ins.opcode == IR::Opcode::basic_cast_short)
                        targetType = compilerCtx->getShortObjectType();
                    else
                        panic(ins.debugInfo.line, ins.debugInfo.column, "Invalid basic cast opcode");
    ...
    AnalysisState IRFunctionOptimizer::analyzeBlockForInterfaceAllocationReduction(indexT blockIndex, const AnalysisState &inState) {
        ...
        auto checkWhetherStackItemIsValidLocalInterfaceObjectAndRecess = [&](yoi::indexT offset = 0) {
            if (simulationStack.peek(offset).type->type == IRValueType::valueType::interfaceObject &&
                simulationStack.peek(offset).metadata.hasMetadata(L"from_load") &&
                simulationStack.peek(offset).metadata.hasMetadata(L"delayed_interface_impl") &&
                simulationStack.peek(offset).metadata.getMetadata<std::pair<yoi::indexT, yoi::indexT>>(L"delayed_interface_impl").first != -1) {
                // valid metadata, optimized local variable, satisify the reverting requirement
                variablesExtraInfo[simulationStack.peek(offset).metadata.getMetadata<yoi::indexT>(L"from_load")].possibleValue.metadata.setMetadata(
                    L"delayed_interface_impl", std::pair<yoi::indexT, yoi::indexT>{-1, -1});
            }
        };
            ...
            switch (ins.opcode) {
                ...
                case IR::Opcode::store_local: {
                    ...
                    if (variablesExtraInfo[ins.operands[0].value.symbolIndex].possibleValue.metadata.hasMetadata(L"delayed_interface_impl") &&
                        value.metadata.hasMetadata(L"delayed_interface_impl")) {
                        auto &impl1 = variablesExtraInfo[ins.operands[0].value.symbolIndex]
                                          .possibleValue.metadata.getMetadata<std::pair<yoi::indexT, yoi::indexT>>(L"delayed_interface_impl");
                        ...
                        variablesExtraInfo[ins.operands[0].value.symbolIndex].possibleValue.metadata.setMetadata(
                            L"delayed_interface_impl", value.metadata.getMetadata<std::pair<yoi::indexT, yoi::indexT>>(L"delayed_interface_impl"));
                    }
                    varType->attributes = value.type->attributes;
                    break;
                }
                case IR::Opcode::construct_interface_impl: {
                    ...
                    simulationStack.pop();
                    simulationStack.push(returnType, {currentCodeBlockIndex, {insIndex}, false});
                    simulationStack.peek(0).metadata.setMetadata(L"delayed_interface_impl",
                                                                 std::pair{moduleIndex, ins.operands[1].value.symbolIndex});
                    break;
                }
                case IR::Opcode::invoke:
                case IR::Opcode::invoke_dangling: {
    ...
    AnalysisState IRFunctionOptimizer::mergeStatesForInterfaceAllocationReduction(const AnalysisState &s1, const AnalysisState &s2) {
        if (s1.variableStates.empty())
            return s2;
        if (s2.variableStates.empty())
            return s1;
        ...
        for (const auto &key : allVarKeys) {
            ...
            if (it1 != s1.variableStates.end() && it2 != s2.variableStates.end()) {
                ...
                if (mergedMetadata.hasMetadata(L"delayed_interface_impl") &&
                    it2->second.possibleValue.metadata.hasMetadata(L"delayed_interface_impl")) {
    ...
    void IRFunctionOptimizer::handleInstruction(const IR &ins, yoi::indexT insIndex, yoi::indexT currentCodeBlockIndex) {
        switch (ins.opcode) {
            ...
            case IR::Opcode::bind_fields_post: {
                ...
                yoi::indexT startPos =
                    ins.opcode == IR::Opcode::bind_fields_post ? 0 : structDef->fieldTypes.size() - ins.operands[0].value.symbolIndex;
                yoi::indexT endPos = startPos + ins.operands[0].value.symbolIndex;
                for (auto i = startPos; i < endPos; ++i) {
                    auto fieldType = structDef->fieldTypes[i];
                    simulationStack.push(fieldType,
                                         structVal.contributedInstructions +
                                             SimulationStack::Item::ContributedInstructionSet{currentCodeBlockIndex, {insIndex}});
                }
                break;
            }
            ...
            case IR::Opcode::new_struct: {
                auto moduleIndex = ins.operands[0].value.symbolIndex;
                auto structDef = compilerCtx->getImportedModule(moduleIndex)->structTable[ins.operands[1].value.symbolIndex];
                simulationStack.push(managedPtr(IRValueType{IRValueType::valueType::structObject, moduleIndex, ins.operands[1].value.symbolIndex}),
                                     {currentCodeBlockIndex, {insIndex}, false});
                break;
            }
            case IR::Opcode::construct_interface_impl: {
    ...
    void IROptimizer::buildCallGraph() {
        ...
        if (compilerCtx->getBuildConfig()->buildType == IRBuildConfig::BuildType::executable) {
            try {
                q.emplace(entryModuleIndex, compilerCtx->getImportedModule(entryModuleIndex)->functionTable.getIndex(L"main#"));
    ...
    bool IRFunctionOptimizer::performParamBorrowCheck() {
        ...
        if (targetFunction->argumentTypes.size()) {
            for (yoi::indexT paramIdx = 0; paramIdx < targetFunction->argumentTypes.size(); paramIdx++) {
                ...
                if (!copied->hasAttribute(IRValueType::ValueAttr::NoBorrow) &&
                    (!targetFunction->hasAttribute(IRFunctionDefinition::FunctionAttrs::Variadic) ||
                     paramIdx != targetFunction->argumentTypes.size() - 1) &&
                    !copied->hasAttribute(IRValueType::ValueAttr::Raw))
    ...
    bool IRFunctionOptimizer::performInterfaceAllocationReduction() {
        ...
        for (const auto &[varIndex, varType] : targetFunction->variableTable.getReversedVariableNameMap()) {
            ...
            for (const auto &[blockIndex, outState] : blockOutStates) {
                if (outState.variableStates.count(varIndex) &&
                    outState.variableStates.at(varIndex).possibleValue.metadata.hasMetadata(L"delayed_interface_impl")) {
                    auto impl = outState.variableStates.at(varIndex).possibleValue.metadata.getMetadata<std::pair<yoi::indexT, yoi::indexT>>(
                        L"delayed_interface_impl");
    ...
    bool IROptimizer::performInterfaceWrapperPass() {
        ...
        for (auto &module : compilerCtx->getCompiledModules()) {
            for (auto &interface : module.second->interfaceTable) {
                // retarded abuse of auto pointers brought about many problems, including
                // where-the-heck-knows-where-the-nullable-is-coming-from things. Thus we completely erase all the tags
                // attached on the interface definitions before we took action.
                ...
                for (auto &impl : interface.second->implementations) {
                    ...
                    auto implName = visitor::getInterfaceImplName(std::pair{module.first, module.second->interfaceTable.getIndex(interface.first)},
                    ...
                    for (yoi::indexT virtIndex = 0; virtIndex < implDef->virtualMethods.size(); virtIndex++) {
                        auto virtDef = compilerCtx->getImportedModule(implDef->virtualMethods[virtIndex]->typeAffiliateModule)
                        ...
                        for (yoi::indexT paramIndex = 0; paramIndex < targetDef->argumentTypes.size(); paramIndex++) {
                            if (virtDef->argumentTypes[paramIndex]->isBasicType() && !virtDef->argumentTypes[paramIndex]->isArrayType() &&
                                !virtDef->argumentTypes[paramIndex]->isDynamicArrayType() &&
                                !virtDef->argumentTypes[paramIndex + 1]->hasAttribute(IRValueType::ValueAttr::Raw)) {
                                targetDef->argumentTypes[paramIndex]
                                    ->removeAttribute(IRValueType::ValueAttr::Raw)
                                    .addAttribute(IRValueType::ValueAttr::Nullable);
                            }
                        }
                        // patch the function return type as well
                        if (virtDef->returnType->isBasicType() && !virtDef->returnType->isArrayType() && !virtDef->returnType->isDynamicArrayType() &&
                            !virtDef->returnType->hasAttribute(IRValueType::ValueAttr::Raw)) {
            ...
            for (auto &interface : module.second->interfaceTable) {
                for (auto &virtIndex : interface.second->methodMap) {
                    ...
                    if (!virtIndex.second->returnType->hasAttribute(IRValueType::ValueAttr::Nullable) &&
                        virtIndex.second->returnType->isBasicType() && virtIndex.second->returnType->dimensions.empty()) {
                        virtIndex.second->returnType->addAttribute(IRValueType::ValueAttr::Raw);
                    }
                }
            }

            auto interface_wrapper_generator = [&](yoi::indexT moduleIndex,
                                                   yoi::indexT funcIndex,
                                                   const yoi::vec<std::shared_ptr<IRValueType>> &targetTypes,
                                                   const std::shared_ptr<IRValueType> &returnType) -> yoi::indexT {
                auto originalFunc = compilerCtx->getImportedModule(moduleIndex)->functionTable[funcIndex];
                auto builder = IRFunctionDefinition::Builder()
                                   .setReturnType(returnType)
                                   .setDebugInfo(originalFunc->debugInfo)
                                   .setName(originalFunc->name + L"#wrapper");
                ...
                moduleCtx->pushIRBuilder(IRBuilder(
                    compilerCtx, compilerCtx->getImportedModule(moduleIndex), compilerCtx->getImportedModule(moduleIndex)->functionTable[index]));
                moduleCtx->getIRBuilder().switchCodeBlock(moduleCtx->getIRBuilder().createCodeBlock());
                moduleCtx->getIRBuilder().loadOp(IR::Opcode::load_local,
                                                 IROperand{IROperand::operandType::index, IROperand::operandValue{yoi::indexT{0}}},
                                                 originalFunc->argumentTypes[0]);
            ...
            // check the implementation of interface methods again for any misalignment, if presents, generate a wrapper
            // method to fix it.
            for (auto &interface : module.second->interfaceTable) {
                for (auto &impl : interface.second->implementations) {
                    ...
                    for (yoi::indexT virtIndex = 0; virtIndex < implDef->virtualMethods.size(); virtIndex++) {
                        ...
                        if (needWrapper)
                            implDef->virtualMethods[virtIndex]->typeIndex =
                                interface_wrapper_generator(implDef->virtualMethods[virtIndex]->typeAffiliateModule,
                                                            implDef->virtualMethods[virtIndex]->typeIndex,
                                                            targetDef->argumentTypes,
                                                            targetDef->returnType);
                    }
                }
            }
        }
    ...
    std::pair<std::map<indexT, std::vector<indexT>>, std::map<indexT, std::vector<indexT>>> IRFunctionOptimizer::performCFGAnalysis() {
        std::map<indexT, std::vector<indexT>> successors;
        std::map<indexT, std::vector<indexT>> predecessors;
    ...
    bool IROptimizer::performBaseOptimization() {
        ...
        for (const auto &[funcId, analysisInfo] : functionAnalysisResults) {
            ...
            for (yoi::indexT i = 0; i < analysisInfo.paramStates.size(); i++) {
                ...
                // we DO NOT modify the original Raw tag.
                switch (analysisInfo.paramStates[i]) {
                    case FunctionAnalysisInfo::ParameterState::Raw: {
                        break;
                    }
                    case FunctionAnalysisInfo::ParameterState::Nullable: {
                        paramType->addAttribute(IRValueType::ValueAttr::Nullable);
                        break;
                    }
                    case FunctionAnalysisInfo::ParameterState::Plain: {
                        break;
        ...
        for (const auto &funcId : callGraph.functions) {
            ...
            optimizer.setTargetFunction(func, funcId).doOptimizationForCurrentFunction();
        }
