...
namespace yoi {
    ...
    std::shared_ptr<yoi::IRModule> visitor::visit() {
        ...
        auto globInitializer = managedPtr(IRFunctionDefinition{L"yoimiya_glob_initializer",
                                                               {},
                                                               moduleContext->getCompilerContext()->getNoneObjectType(),
                                                               {},
                                                               {IRFunctionDefinition::FunctionAttrs::Preserve},
                                                               debugInfo});
    ...
    yoi::indexT visitor::visit(yoi::basicLiterals *basicLiterals) {
        switch (basicLiterals->node.kind) {
            ...
            case yoi::lexer::token::tokenKind::character: {
                moduleContext->getIRBuilder().pushOp(IR::Opcode::push_character, {IROperand::operandType::character, basicLiterals->node.strVal[0]});
                break;
            }
            case yoi::lexer::token::tokenKind::shortInt: {
                moduleContext->getIRBuilder().pushOp(IR::Opcode::push_short, {IROperand::operandType::shortInt, basicLiterals->node.basicVal.vShort});
                break;
            }
            case yoi::lexer::token::tokenKind::unsignedInt: {
                moduleContext->getIRBuilder().pushOp(IR::Opcode::push_unsigned,
                                                     {IROperand::operandType::unsignedInt, basicLiterals->node.basicVal.vUint});
                break;
            }
    ...
    yoi::indexT visitor::visit(yoi::primary *primary, bool isStoreOp) {
        switch (primary->kind) {
            ...
            case primary::primaryKind::lambdaExpr: {
                ...
                auto [lambdaCallableIndex, callableInterface] =
                    createCallableImplementationForLambda(irModule->structTable[lambdaStructIndex], lambdaStructIndex, currentModuleIndex);
        ...
        if (abstractExpr->rhs) {
            ...
            if (abstractExpr->op.kind == lexer::token::tokenKind::kImpl) {
                yoi_assert(rhs.type == IRValueType::valueType::interfaceObject,
                           abstractExpr->rhs->getLine(),
                           abstractExpr->rhs->getColumn(),
                           "RHS of 'impl' operator must be an interface type");
                ...
                // push the boolean
                if (moduleContext->getCompilerContext()
                        ->getImportedModule(lhs->typeAffiliateModule)
                        ->interfaceImplementationTable.contains(interfaceImplName)) {
                ...
                yoi_assert(lhs->type == IRValueType::valueType::interfaceObject,
                           abstractExpr->lhs->getLine(),
                           abstractExpr->lhs->getColumn(),
                           "LHS of 'interfaceof' operator must be an interface type");
                // cut off some possibilies here.
                auto key = std::make_tuple(rhs.type, rhs.typeAffiliateModule, rhs.typeIndex);
                auto &vec =
                    moduleContext->getCompilerContext()->getImportedModule(lhs->typeAffiliateModule)->interfaceTable[lhs->typeIndex]->implementations;
    ...
    yoi::indexT visitor::visit(yoi::equalityExpr *equalityExpr) {
        ...
        for (; op != equalityExpr->getOp().end(); ++op) {
            ...
            if (lhsType->isBasicType() && rhsType->isBasicType() || lhsType->type == IRValueType::valueType::pointerObject ||
                rhsType->type == IRValueType::valueType::pointerObject) {
                switch (op->kind) {
    ...
    yoi::indexT visitor::visit(yoi::logicalAndExpr *logicalAndExpr) {
        ...
        if (termType->type != IRValueType::valueType::booleanObject) {
            moduleContext->getIRBuilder().basicCast(moduleContext->getCompilerContext()->getBoolObjectType(), {}, true);
        }
        ...
        moduleContext->getIRBuilder().insert({IR::Opcode::push_boolean,
                                              {{IROperand::operandType::boolean, IROperand::operandValue{false}}},
                                              moduleContext->getIRBuilder().getCurrentDebugInfo()});
        moduleContext->getIRBuilder().jumpOp(endBlock);
    ...
    yoi::indexT visitor::visit(yoi::memberExpr *memberExpr, bool isStoreOp) {
        ...
        if (it + 2 == memberExpr->getTerms().end()) {
            ...
            if ((*it)->isIdentifier() && !(*it)->id->hasTemplateArg() && (*(it + 1))->isIdentifier() && !(*(it + 1))->id->hasTemplateArg() &&
                targetedModule->enumerationTable.contains((*it)->id->id->node.strVal)) {
                try {
                    ...
                    panic((*(it + 1))->getLine(),
                          (*(it + 1))->getColumn(),
                          "Enumeration value not found: " + yoi::wstring2string((*(it + 1))->id->id->node.strVal));
                }
            }
        }
            ...
            if (currentTermNode->getSubscript().empty()) {
                    ...
                    try {
                        ...
                        if (nameInfo.type != IRStructDefinition::nameInfo::nameType::field) {
                            panic(currentTermNode->getLine(),
                                  ...
                                  "Cannot access method '" + wstring2string(memberName) + "' as a field.");
                        }
                ...
                if (isMethodCall) {
                    if (objectType->type == IRValueType::valueType::structObject) {
                        ...
                        isResolved = handleInvocationExtern(
                            currentTermNode->id->getId().get().strVal, firstOp->args, objectType->typeAffiliateModule, objectType, false,
                            currentTermNode->id->hasTemplateArg() ? &currentTermNode->id->getArg() : nullptr);

                        if (!isResolved) {
                            auto structDef = moduleContext->getCompilerContext()
                                                 ->getImportedModule(objectType->typeAffiliateModule)
                                                 ->structTable[objectType->typeIndex];
                            try {
                                // no method call would ever enter this block
                                auto info = structDef->lookupName(currentTermNode->id->getId().get().strVal);
                                moduleContext->getIRBuilder().loadMemberOp({IROperand::operandType::index, info.index},
                                                                           structDef->fieldTypes[info.index]);

                                yoi_assert(structDef->fieldTypes[info.index]->type == IRValueType::valueType::structObject ||
                                               structDef->fieldTypes[info.index]->type == IRValueType::valueType::interfaceObject,
                                           currentTermNode->getLine(),
                                           currentTermNode->getColumn(),
                                           "Cannot invoke basic types as methods: " + wstring2string(currentTermNode->id->getId().get().strVal));
                                if (!handleInvocationExtern(L"operator()",
                                                            firstOp->args,
                                                            structDef->fieldTypes[info.index]->typeAffiliateModule,
                                                            structDef->fieldTypes[info.index]))
                                    panic(currentTermNode->getLine(),
                                          currentTermNode->getColumn(),
                                          "No matching method found for: " + wstring2string(currentTermNode->id->getId().get().strVal) +
                                              ".operator()");
                        ...
                        if (!handleInvocationExtern(
                                currentTermNode->id->getId().get().strVal, firstOp->args, objectType->typeAffiliateModule, objectType))
                            panic(currentTermNode->getLine(),
                                  currentTermNode->getColumn(),
                                  "No matching method found for: " + wstring2string(currentTermNode->id->getId().get().strVal));
                    }
                    ...
                    if (objectType->type == IRValueType::valueType::structObject) {
                        ...
                        try {
                            ...
                            if (nameInfo.type != IRStructDefinition::nameInfo::nameType::field) {
                                panic(currentTermNode->getLine(),
                                      currentTermNode->getColumn(),
                                      "Cannot subscript method '" + wstring2string(fieldName) + "'.");
                            }
                            auto fieldType = structDef->fieldTypes[nameInfo.index];
                            moduleContext->getIRBuilder().loadMemberOp({IROperand::operandType::index, nameInfo.index}, fieldType);
                        } catch (std::out_of_range &) {
                            panic(currentTermNode->getLine(),
                                  currentTermNode->getColumn(),
                                  "Struct '" + wstring2string(structDef->name) + "' has no field named '" + wstring2string(fieldName) + "'.");
                        }
                    } else {
                ...
                for (; subIt != currentTermNode->getSubscript().end(); ++subIt) {
                    ...
                    if (sub->isInvocation()) {
                        ...
                        if (!handleInvocationExtern(L"operator()", sub->args, currentObjectType->typeAffiliateModule, currentObjectType))
                            panic(sub->getLine(),
                                  sub->getColumn(),
                                  "No matching method found for: " + wstring2string(currentTermNode->id->getId().get().strVal) + ".operator()");
                        ...
                        if (currentObjectType->isArrayType() || currentObjectType->isDynamicArrayType()) {
                            ...
                            yoi_assert(indexType->type == IRValueType::valueType::unsignedObject,
                                       sub->getLine(),
                                       sub->getColumn(),
                            ...
                            if (isFinalOperation && isStoreOp) {
                                ...
                                yoi_assert(overload.found(), sub->getLine(), sub->getColumn(), "No matching overload found for operator[].");
                                yoi_assert(
                                    !overload.isVariadic, sub->getLine(), sub->getColumn(), "Variadic operator[] overloading is not supported.");
    ...
    yoi::indexT visitor::visit(yoi::subscriptExpr *subscriptExpr, bool isStoreOp) {
        ...
        if (isType && first_term->isSubscript()) {
            ...
            while (dim_it != end && (*dim_it)->isSubscript()) {
                yoi_assert((*dim_it)->expr->getToken().kind == lexer::token::tokenKind::integer,
                           (*dim_it)->expr->getLine(),
                           (*dim_it)->expr->getColumn(),
        ...
        // Case 2: Invocation `id<...>(...)` or `id(...)`
                ...
                if (irModule->funcTemplateAsts.contains(baseName)) {
                    ...
                    try {
                        ...
                        switch (baseType->type) {
                            case IRValueType::valueType::structObject:
                                baseName = moduleContext->getCompilerContext()
                                               ->getImportedModule(baseType->typeAffiliateModule)
                                               ->structTable.getKey(baseType->typeIndex);
                                break;
                            case IRValueType::valueType::interfaceObject:
                                baseName = moduleContext->getCompilerContext()
                                               ->getImportedModule(baseType->typeAffiliateModule)
                                               ->interfaceTable.getKey(baseType->typeIndex);
                                break;
                            default:
                                panic(subscriptExpr->getLine(),
                                      subscriptExpr->getColumn(),
                                      "Cannot specialize type: except structObject or interfaceObject but got: " +
                                          yoi::wstring2string(baseType->to_string()));
                                break;
                        }
                    } catch (const std::runtime_error &e) {
                        panic(subscriptExpr->getLine(),
                              subscriptExpr->getColumn(),
                              "Could not resolve template specialization for function '" + wstring2string(baseName) + ": \n" + e.what());
                    }
                }
            }
            ...
            moduleContext->getIRBuilder().saveState();
            {
                ...
                if (isAccessible) {
                    ...
                    if (structObject->type == IRValueType::valueType::structObject || structObject->type == IRValueType::valueType::interfaceObject) {
                        ...
                        panic(
                            subscriptExpr->getLine(), subscriptExpr->getColumn(), "Cannot call operator() on non-struct object or interface object.");
                    }
                }
            }
            ...
            if (irModule->structTable.contains(baseName)) {
                ...
                if (handleInvocationExtern(L"constructor", args, currentModuleIndex, rhs)) {
                    ...
                    panic(subscriptExpr->getLine(),
                          subscriptExpr->getColumn(),
                          "Could not resolve constructor for struct '" + wstring2string(baseName) + "'.");
                }
            } else {
                moduleContext->getIRBuilder().restoreState();
            }
                ...
                if (irModule->interfaceTable.contains(baseName)) {
                    ...
                    try {
                        ...
                        yoi_assert(argTypes.size() == 1,
                                   ...
                                   subscriptExpr->getColumn(),
                                   "Interface constructor expects exactly one argument.");
                        ...
                        moduleContext->getIRBuilder().constructInterfaceImplOp(
                ...
                if (irModule->externTable.contains(baseName)) {
                    ...
                    try {
                        ...
                        yoi_assert(irModule->externTable[importedFunctionIndex]->type == IRExternEntry::externType::importedFunction,
                                   subscriptExpr->getLine(),
                                   subscriptExpr->getColumn(),
                                   "This is not an imported function.");
                        auto importedFunc = moduleContext->getCompilerContext()
                                                ->getIRFFITable()
                        ...
                        yoi_assert(desiredArgTypes.size() == args->arg.size(),
                                   args->getLine(),
                                   args->getColumn(),
                        ...
                        moduleContext->getIRBuilder().invokeImportedOp(irModule->externTable[importedFunctionIndex]->affiliateModule,
            ...
            if (!resolved) {
                panic(subscriptExpr->getLine(),
                      subscriptExpr->getColumn(),
                      "Could not resolve call to '" + wstring2string(baseName) +
                          "'. No matching function, constructor, or template found for the given arguments.");
            }
        }
        ...
        while (it != end) {
            ...
            if (currentTerm->isSubscript()) {
                if (handleSubscript(it, end, isStoreOp, isLastTerm))
                    continue;
                else
                    return moduleContext->getIRBuilder().getCurrentInsertionPoint(); // avoid releasing the state twice.
            } else if (currentTerm->isInvocation()) {
                if (objectOnStackType->type == IRValueType::valueType::structObject ||
                    objectOnStackType->type == IRValueType::valueType::interfaceObject) {
                    if (!handleInvocationExtern(L"operator()", currentTerm->args, currentModuleIndex, objectOnStackType))
                        panic(currentTerm->getLine(),
                              ...
                              "No matching method found for: " + wstring2string(subscriptExpr->id->getId().get().strVal));
                } else {
    ...
    yoi::indexT visitor::visitExtern(yoi::subscriptExpr *subscriptExpr, yoi::indexT targetModule, bool isStoreOp) {
        ...
        // Case 2: Extern Invocation
            ...
            if (!resolved) {
                if (targetedModule->interfaceTable.contains(baseName)) {
                    ...
                    try {
                        ...
                        auto interfaceImplIndex = moduleContext->getCompilerContext()
                                                      ->getImportedModule(concreteThis->typeAffiliateModule)
                                                      ->interfaceImplementationTable.getIndex(interfaceImplName);
                        moduleContext->getIRBuilder().constructInterfaceImplOp({externInterface.affiliateModule, externInterface.itemIndex},
                                                                               interfaceImplIndex,
                                                                               concreteThis->typeAffiliateModule != currentModuleIndex,
                                                                               concreteThis->typeAffiliateModule);
                        ...
                        panic(subscriptExpr->getLine(),
                              subscriptExpr->getColumn(),
                              "Could not find matched extern interface constructor for " + wstring2string(baseName));
                    } /* catch (const std::exception &e) {
                         // panic(subscriptExpr->getLine(), subscriptExpr->getColumn(), "Could not find matched extern interface constructor for " +
                     wstring2string(baseName)); throw e;
                     }*/
                }
            }
            ...
            if (!resolved) {
                if (targetedModule->externTable.contains(baseName)) {
                    ...
                    try {
                        ...
                        auto externFunc = moduleContext->getCompilerContext()
                                              ->getIRFFITable()
                                              ->importedLibraries[externEntry->affiliateModule]
                                              .importedFunctionTable[externEntry->itemIndex];
                        moduleContext->getIRBuilder().invokeImportedOp(
                            externEntry->affiliateModule, externEntry->itemIndex, argTypes.size(), externFunc->returnType);
            ...
            if (!resolved) {
                panic(subscriptExpr->getLine(),
                      subscriptExpr->getColumn(),
                      "Could not find extern function, struct constructor, or interface in module: " + wstring2string(baseName));
            }
        ...
        while (it != end) {
            ...
            if (currentTerm->isSubscript()) {
                if (handleSubscript(it, end, isStoreOp, isLastTerm))
                    ...
                    return moduleContext->getIRBuilder().getCurrentInsertionPoint(); // avoid releasing state twice
            } else if (currentTerm->isInvocation()) {
                if (objectOnStackType->type == IRValueType::valueType::structObject ||
                    objectOnStackType->type == IRValueType::valueType::interfaceObject) {
    ...
    IRValueType visitor::parseTypeSpec(yoi::identifier *identifier) {
        ...
        try {
            ...
            return IRValueType{IRValueType::valueType::interfaceObject, static_cast<yoi::indexT>(currentModuleIndex), typeIndex};
        } catch (std::out_of_range &e) {
            // let it go
        }
        try {
        ...
        if (funcName.hasDefTemplateArg()) {
            ...
            for (auto &i : funcDefStmt->getArgs().get()) {
                if (&i == &funcDefStmt->getArgs().get().back() && i->spec->kind == 3 /* elipsis */) {
                    ...
                    auto argType = managedPtr(i->spec->elipsis ? parseTypeSpec(i->spec->elipsis).getDynamicArrayType()
    ...
    yoi::indexT visitor::visit(yoi::interfaceDefStmt *interfaceDefStmt) {
        ...
        for (auto &i : interfaceDefStmt->getInner().getInner()) {
            ...
            auto methodName = i->getMethod().getName().getId().get().strVal;
            ...
            for (auto &arg : i->getMethod().getArgs().get()) {
                if (&arg == &i->getMethod().getArgs().get().back() && arg->spec->kind == 3 /* elipsis */) {
                    ...
                    auto argType =
                        managedPtr(arg->spec->elipsis ? parseTypeSpec(arg->spec->elipsis).getDynamicArrayType()
                                                      : moduleContext->getCompilerContext()->getNullInterfaceType()->getDynamicArrayType());
            ...
            for (auto &i : structDefStmt->getInner().getInner()) {
                switch (i->kind) {
                    ...
                    case 1: {
                        ...
                        for (auto &arg : i->getConstructor().getArgs().get()) {
                            if (&arg == &i->getConstructor().getArgs().get().back() && arg->spec->kind == 3 /* elipsis */) {
                                ...
                                auto argName = arg->getId().node.strVal;
                                auto argType = managedPtr(arg->spec->elipsis
                    ...
                    case 2: {
                        ...
                        if (i->getMethod().getName().hasDefTemplateArg()) {
                            // template method declaration
                            builder.addTemplateMethodDecl(methodName, i);
                            break;
                        }
                        ...
                        if (std::find(methodBuilder.attrs.begin(), methodBuilder.attrs.end(), IRFunctionDefinition::FunctionAttrs::Static) ==
                            methodBuilder.attrs.end())
    ...
    yoi::indexT visitor::visit(yoi::implStmt *implStmt) {
        ...
        yoi_assert(it + 1 == structIdNode.getTerms().end(), structIdNode.getLine(), structIdNode.getColumn(), "Invalid interface name");
        ...
        if ((*it)->hasTemplateArg()) {
            ...
            if (implStmt->isImplForStmt()) {
                ...
                if (concreteTemplateArgs.empty()) {
                    ...
                    yoi_assert(concreteStructType->type == IRValueType::valueType::structObject,
                               implStmt->getLine(),
                               implStmt->getColumn(),
                               "Invalid struct name for struct specialization: " + wstring2string(concreteStructName) +
                                   " (except structObject but got " + wstring2string(concreteStructType->to_string()) + ")");
                    // FIXED: module index
        ...
        if (implStmt->isImplForStmt()) {
            ...
            auto targetInterface =
                moduleContext->getCompilerContext()->getImportedModule(interfaceName.first.first)->interfaceTable[interfaceName.first.second];
            targetInterface->implementations.emplace_back(srcType->type, srcType->typeAffiliateModule, srcType->typeIndex);
            auto interfaceImplName = getInterfaceImplName(interfaceName.first, srcType);
            ...
            try {
                ...
                if (targetedModule->interfaceImplementationTable[interfaceImplIndex]) {
                    panic(
                        implStmt->getLine(), implStmt->getColumn(), "Redefinition of interface implementation: " + wstring2string(interfaceImplName));
                }
            } catch (std::out_of_range &e) {
            ...
            if (implStmt->inner) {
                ...
                for (auto &i : implStmt->getInner().getInner()) {
                    ...
                    targetedModule->functionOverloadIndexies[structBaseName + L"::" + methodName + L"interfaceImpl#" + interfaceImplName].push_back(
                        funcIndex);
                    /*builder.addVirtualMethod(
                        methodName + uniq,
                        managedPtr(IRValueType{IRValueType::valueType::virtualMethod, targetModule, funcIndex}));*/
                    virtualMethodMap[methodName + getFuncUniqueNameStr(argTypes, true)] = {
                        methodName + uniq, managedPtr(IRValueType{IRValueType::valueType::virtualMethod, targetModule, funcIndex})};
                ...
                for (auto &method : targetInterface->methodMap) {
                    yoi_assert(virtualMethodMap.contains(method.first),
                               implStmt->getLine(),
                               implStmt->getColumn(),
                               "Method '" + wstring2string(method.first) + "' not implemented for interface '" +
                                   wstring2string(targetInterface->name) + "'");
            ...
            for (auto &i : implStmt->getInner().getInner()) {
                ...
                if (i->isConstructor()) {
                    ...
                    if (i->getMethod().getName().hasTemplateArg()) {
                        // template method definition for a non-template struct
                        auto structDef = targetedModule->structTable[structIndex];
                        structDef->templateMethodDefs[i->getMethod().getName().getId().get().strVal] = i;
                        continue;
                    }
                    ...
                    mangledName = structBaseName + L"::" + i->getMethod().getName().getId().get().strVal + getFuncUniqueNameStr(argTypes);
                ...
                try {
                    ...
                    panic(
                        i->getLine(), i->getColumn(), "No matched constructor or method declaration found for impl: " + wstring2string(mangledName));
                }
            }
        }
        return moduleContext->getIRBuilder().getCurrentInsertionPoint();
        ...
        for (auto &i : letStmt->terms) {
            ...
            if (i->lhs->kind == letAssignmentPairLHS::vKind::identifier) {
                if (isVisitingGlobalScope()) {
                    ...
                    tryCastTo(type);
                    auto index = moduleContext->getIRBuilder().irFuncDefinition()->getVariableTable().put(i->lhs->id->node.strVal, type);
                ...
                if (type->isArrayType() || type->isDynamicArrayType()) {
                    // array binding
                    IRBuilder::ExtractType extractType{i->lhs->list.back().kind == lexer::token::tokenKind::kThreeDots
                                                           ? IRBuilder::ExtractType::First
                                                           : IRBuilder::ExtractType::Last};
                    bool isFull = i->lhs->list.back().kind != lexer::token::tokenKind::kThreeDots &&
                                  i->lhs->list.front().kind != lexer::token::tokenKind::kThreeDots;
                    ...
                    for (yoi::indexT curPos = extractType == IRBuilder::ExtractType::First && !isFull; curPos < elementCount; curPos++) {
                        if (isVisitingGlobalScope()) {
                            ...
                            auto index = moduleContext->getIRBuilder().irFuncDefinition()->getVariableTable().put(
                                i->lhs->list[i->lhs->list.size() - 1 - curPos].strVal, bindType);
                            moduleContext->getIRBuilder().storeOp(IR::Opcode::store_local, {IROperand::operandType::localVar, index});
                        }
                    }
    ...
    IRValueType visitor::parseTypeSpec(yoi::identifierWithTemplateArg *identifierWithTemplateArg) {
        if (identifierWithTemplateArg->hasTemplateArg()) {
            ...
            if (irModule->structTemplateAsts.contains(baseName)) {
                ...
                panic(identifierWithTemplateArg->getLine(), identifierWithTemplateArg->getColumn(), "Unknown template: " + wstring2string(baseName));
            }
        } else {
    ...
    IRValueType visitor::parseTypeSpec(yoi::subscriptExpr *subscriptExpr) {
        if (!subscriptExpr->getSubscript().empty()) {
            ...
            for (auto &sub : subscriptExpr->getSubscript()) {
                yoi_assert(sub->isSubscript() && sub->expr->getToken().kind == lexer::token::tokenKind::integer,
                           sub->getLine(),
                           sub->getColumn(),
                           "Expected dimension size for array type.");
    ...
    IRValueType visitor::parseTypeSpecExtern(yoi::identifierWithTemplateArg *identifierWithTemplateArg, yoi::indexT targetModule) {
        if (identifierWithTemplateArg->hasTemplateArg()) {
            auto targetedModule = moduleContext->getCompilerContext()->getImportedModule(targetModule);
    ...
    IRValueType visitor::parseTypeSpec(yoi::typeSpec *typeSpec) {
        switch (typeSpec->kind) {
            case 0: {
                ...
                yoi::indexT targetModule = -1, lastModule = -1;
                while (it + 1 != typeSpec->member->getTerms().end() && (targetModule = isModuleName(*it, targetModule)) != lastModule) {
                    it++;
                    lastModule = targetModule;
                }
    ...
    yoi::wstr visitor::getInterfaceImplName(const std::pair<yoi::indexT, yoi::indexT> &interfaceSrc, const std::shared_ptr<IRValueType> &typeSrc) {
        return L"interfaceImpl#" + std::to_wstring(interfaceSrc.first) + L"#" + std::to_wstring(interfaceSrc.second) + L"#" + typeSrc->to_string();
    }
    ...
    yoi::IRExternEntry visitor::getExternEntry(yoi::indexT moduleIndex, const yoi::wstr &identifier) const {
        ...
        throw std::out_of_range("undefined identifier: not known global variable, function, struct or interface type: " +
    ...
    void visitor::emitBasicCastInBasicArithOpByLhsAndRhs(yoi::indexT lhs, yoi::indexT rhs) {
        ...
        else if (lhsType->type == IRValueType::valueType::unsignedObject && rhsType->type == IRValueType::valueType::decimalObject) {
            moduleContext->getIRBuilder().basicCast(rhsType, lhs, true);
    ...
    yoi::indexTable<yoi::wstr, IRTemplateBuilder::Argument> visitor::getTemplateArgs(const yoi::defTemplateArg &templateArgs) {
    ...
    yoi::indexT visitor::specializeFunctionTemplate(yoi::funcDefStmt *astNode,
        ...
        yoi::wstr specializedName = getMangledTemplateName(astNode->id->id->get().strVal, concreteTemplateArgs);
        ...
        try {
            ...
            panic(astNode->getLine(),
                  astNode->getColumn(),
                  std::string("Exception occurred while specializing method: ") + yoi::wstring2string(specializedName) + ": " + e.what());
        } catch (std::exception &e) {
            panic(astNode->getLine(),
                  astNode->getColumn(),
                  std::string("Unknown exception occurred while specializing method: ") + yoi::wstring2string(specializedName) + ": " + e.what());
        }
        moduleContext->getIRBuilder().yield();
        moduleContext->popIRBuilder();
    ...
    yoi::vec<yoi::wstr> visitor::extractTemplateParamsFromTypeArgs(yoi::templateArg *templateArgs) {
        yoi::vec<yoi::wstr> params;
        if (!templateArgs)
            return params;
        for (auto &spec : templateArgs->get()) {
            ...
            // If complex type or anything else, it's invalid for a definition param
            panic(templateArgs->getLine(), templateArgs->getColumn(), "Invalid template parameter definition. Expected identifier.");
        }
        return params;
    }

    yoi::vec<yoi::wstr> visitor::extractTemplateParamsFromTypeArgs(yoi::defTemplateArg *templateArgs) {
        yoi::vec<yoi::wstr> params;
        if (!templateArgs)
            return params;
        for (auto &spec : templateArgs->get()) {
            params.push_back(spec->getId().get().strVal);
        }
        return params;
    }
    ...
    yoi::indexT visitor::specializeStructTemplate(const yoi::wstr &templateName,
        ...
        yoi::vec<yoi::wstr> paramNames;
        for (yoi::indexT i = 0; i < structAst->id->getArg().get().size(); ++i) {
            paramNames.push_back(structAst->id->getArg().get()[i]->getId().get().strVal);
        }
        builder.setStoredTemplateArgs(paramNames, concreteTemplateArgs);
        for (auto &field : structAst->getInner().getInner()) {
            if (field->kind == 0) {
                ...
                builder.addTemplateMethodDecl(field->getMethod().getName().getId().get().strVal, field);
            } else {
                auto [funcIndex, funcName] =
                    specializeStructMethodDeclaration(specializationContext, field, specializedName, concreteTemplateArgs, moduleIndex);
                builder.addMethod(funcName, funcIndex);
            }
        }

        // Collect generic method definitions from the pure template impl
        if (pureTemplateImplAst) {
            for (auto &methodAst : pureTemplateImplAst->getInner().getInner()) {
                if (methodAst->isMethod() && methodAst->getMethod().getName().hasTemplateArg()) {
                    builder.addTemplateMethodDef(methodAst->getMethod().getName().getId().get().strVal, methodAst);
                }
            }
        }
        ...
        if (pureTemplateImplAst) {
            for (auto &methodAst : pureTemplateImplAst->getInner().getInner()) {
                if (methodAst->isMethod() && methodAst->getMethod().getName().hasTemplateArg()) {
                    // Handled above for templateMethodDefs
                    continue;
                } else {
                    specializeStructMethodDefinition(
                        specializationContext, specializedStruct, methodAst, specializedName, concreteTemplateArgs, moduleIndex);
                }
            }
        }
        ...
        if (targetedModule->templateInterfaceImplAsts.count(templateName)) {
            for (auto &implAst : targetedModule->templateInterfaceImplAsts.at(templateName)) {
                specializeInterfaceImplementation(
                    implAst, selfType, specializedName, concreteTemplateArgs, currentModuleIndex); // now we are in the specialized context
            }
        }
    ...
    yoi::indexT visitor::specializeStructMethodTemplate(const std::shared_ptr<IRStructDefinition> &structDef,
        ...
        // Combine struct template args and method template args
        IRTemplateBuilder combinedContext;
        // Reconstruct struct specialization context
        for (size_t i = 0; i < structDef->templateParamNames.size(); ++i) {
            combinedContext.addTemplateArgument(structDef->templateParamNames[i], structDef->storedTemplateArgs[i]);
        }

        // Add method template args
        yoi::vec<yoi::wstr> methodParams;
        if (decl) {
            methodParams = extractTemplateParamsFromTypeArgs(&decl->getMethod().getName().getArg());
        } else if (def) {
            methodParams = extractTemplateParamsFromTypeArgs(&def->getMethod().getName().getArg());
        }
        ...
        yoi::wstr specializedMethodName = getMangledTemplateName(baseMethodName, methodTemplateArgs);

        pushModuleContext(moduleIndex);
        moduleContext->pushTemplateBuilder(combinedContext);

        // Reuse specializeStructMethodDeclaration logic but with combined context
        IRFunctionDefinition::Builder funcBuilder;
        funcBuilder.setDebugInfo({targetedModule->modulePath, (decl ? decl->getLine() : def->getLine()), (decl ? decl->getColumn() : def->getColumn())});

        auto selfType = managedPtr(IRValueType{IRValueType::valueType::structObject, moduleIndex, targetedModule->structTable.getIndex(structDef->name)});
        combinedContext.addTemplateArgument(L"STRUCT", selfType);

        yoi::vec<std::shared_ptr<IRValueType>> specializedArgTypes;
        if (decl) {
            funcBuilder.attrs = getFunctionAttributes(decl->getMethod().attrs);
            if (std::find(funcBuilder.attrs.begin(), funcBuilder.attrs.end(), IRFunctionDefinition::FunctionAttrs::Static) == funcBuilder.attrs.end()) {
                funcBuilder.addArgument(L"this", selfType);
            }
            for (auto &arg : decl->getMethod().getArgs().get()) {
            ...
            funcBuilder.setReturnType(managedPtr(parseTypeSpec(&decl->getMethod().getResultType())));
        } else {
            // If only definition exists...
            funcBuilder.attrs = getFunctionAttributes(def->getMethod().attrs);
             if (std::find(funcBuilder.attrs.begin(), funcBuilder.attrs.end(), IRFunctionDefinition::FunctionAttrs::Static) == funcBuilder.attrs.end()) {
                funcBuilder.addArgument(L"this", selfType);
            }
            for (auto &arg : def->getMethod().getArgs().get()) {
                auto specializedType = managedPtr(parseTypeSpec(&arg->getSpec()));
                funcBuilder.addArgument(arg->getId().get().strVal, specializedType);
                specializedArgTypes.push_back(specializedType);
            }
            funcBuilder.setReturnType(managedPtr(parseTypeSpec(&def->getMethod().getResultType())));
        }

        yoi::wstr fullMangledName = structDef->name + L"::" + specializedMethodName + getFuncUniqueNameStr(specializedArgTypes);

        if (targetedModule->functionTable.contains(fullMangledName)) {
            auto res = targetedModule->functionTable.getIndex(fullMangledName);
            moduleContext->popTemplateBuilder();
            popModuleContext();
            return res;
        }

        funcBuilder.setName(fullMangledName);
        auto specializedFunc = funcBuilder.yield();
        auto funcIndex = targetedModule->functionTable.put_create(fullMangledName, specializedFunc);
        targetedModule->functionOverloadIndexies[structDef->name + L"::" + baseMethodName].push_back(funcIndex);

        // If definition exists, visit it
        if (def) {
            moduleContext->pushIRBuilder({moduleContext->getCompilerContext(), targetedModule, specializedFunc});
            moduleContext->getIRBuilder().setDebugInfo({targetedModule->modulePath, def->getLine(), def->getColumn()});
            moduleContext->getIRBuilder().switchCodeBlock(moduleContext->getIRBuilder().createCodeBlock());
            try {
                visit(&def->getMethod().getBlock(), true);
            } catch (std::exception &e) {
                panic(def->getLine(), def->getColumn(), std::string("Exception occurred while specializing template method: ") + yoi::wstring2string(fullMangledName) + ": " + e.what());
            }
            moduleContext->getIRBuilder().yield();
            moduleContext->popIRBuilder();
        }
    ...
    std::pair<yoi::indexT, yoi::wstr> visitor::specializeStructMethodDeclaration(IRTemplateBuilder &structTemplate,
        ...
        auto selfType =
            managedPtr(IRValueType{IRValueType::valueType::structObject, moduleIndex, targetedModule->structTable.getIndex(specializedStructName)});
        specializationContext.addTemplateArgument(L"STRUCT", selfType);
        moduleContext->pushTemplateBuilder(specializationContext);
        ...
        if (methodAstNode->kind == 1) {
            ...
            if (std::find(funcBuilder.attrs.begin(), funcBuilder.attrs.end(), IRFunctionDefinition::FunctionAttrs::Static) ==
                funcBuilder.attrs.end()) {
                funcBuilder.addArgument(L"this", selfType); // Specialized 'this'
            }
            for (auto &arg : methodAstNode->getMethod().getArgs().get()) {
                auto specializedType = managedPtr(parseTypeSpec(&arg->getSpec()));
        ...
        yoi::wstr specializedMethodName = specializedStructName + L"::" + baseMethodName;
    ...
    void visitor::specializeStructMethodDefinition(IRTemplateBuilder &structTemplate,
        ...
        try {
            ...
            panic(moduleContextStack.top().first->getIRBuilder().getCurrentDebugInfo().line,
                  moduleContextStack.top().first->getIRBuilder().getCurrentDebugInfo().column,
                  std::string("Exception occurred while specializing method: ") + yoi::wstring2string(specializedMethodName) + ": " + e.what() +
                      "\n");
        }
    ...
    yoi::wstr visitor::getSpecializedMangledMethodName(yoi::indexTable<yoi::wstr, IRTemplateBuilder::Argument> &templateArgs,
                                                       const yoi::wstr &baseMethodName,
    ...
    yoi::indexT visitor::visit(yoi::exportDecl *exportDecl) {
        ...
        try {
            ...
            yoi::indexT targetModule = -1, lastModule = -1;
            while (it + 1 != exportDecl->from->member->getTerms().end() && (targetModule = isModuleName(*it, targetModule)) != lastModule) {
                it++;
                lastModule = targetModule;
            }
            ...
            if (!(*it)->hasTemplateArg()) {
                for (auto funcIt = moduleContext->getCompilerContext()->getImportedModule(targetModule)->functionTable.begin();
                     funcIt != moduleContext->getCompilerContext()->getImportedModule(targetModule)->functionTable.end();
                     funcIt++) {
                    if (funcIt->first.starts_with((*it)->id->get().strVal + L"#")) {
                        // an mangled name of target function
                        funcIndex =
                            std::distance(moduleContext->getCompilerContext()->getImportedModule(targetModule)->functionTable.begin(), funcIt);
                    }
                }
            }
            ...
            if (funcIndex != -1) {
                ...
                moduleContext->getCompilerContext()->getIRFFITable()->addExportedFunction(exportIdentifier, targetModule, funcIndex, attrs);
                return moduleContext->getIRBuilder().getCurrentInsertionPoint();
            } else {
                // try template
                ...
                auto funcIndex = specializeFunctionTemplate(
                    moduleContext->getCompilerContext()->getImportedModule(targetModule)->funcTemplateAsts[templateName], templateArgs, targetModule);
            ...
            panic(exportDecl->getLine(),
                  exportDecl->getColumn(),
                  "None of the existing types and functions match the name: " + wstring2string(exportDecl->as->node.strVal));
        }
    }
    ...
    yoi::indexT visitor::visit(yoi::importDecl *importDecl) {
        ...
        yoi_assert(
            !from.empty(), importDecl->getLine(), importDecl->getColumn(), "Cannot find the file: " + wstring2string(importDecl->from_path.strVal));

        // import method implementation
        // parse method signature and add to import table
        auto funcName = importDecl->inner->name->getId().get().strVal;
        IRFunctionDefinition::Builder builder;
        ...
        irModule->externTable.put_create(
            funcName,
            managedPtr(IRExternEntry{IRExternEntry::externType::importedFunction,
                                     funcName,
    ...
    void visitor::tryCastTo(const std::shared_ptr<IRValueType> &toType) {
        ...
        if (*rhs == *toType) {
            return;
                   ...
                   rhs->type == IRValueType::valueType::null || toType->type == IRValueType::valueType::pointerObject ||
                   toType->type == IRValueType::valueType::pointer) {
            ...
            return;
                   ...
                   !rhs->isArrayType() && !toType->isArrayType() &&
                   (rhs->type != IRValueType::valueType::stringObject || toType->type == IRValueType::valueType::pointerObject)) {
            ...
            try {
                ...
                auto implIndex =
                    moduleContext->getCompilerContext()->getImportedModule(rhs->typeAffiliateModule)->interfaceImplementationTable.getIndex(implName);
                // construct interface object
                // moduleContext->getIRBuilder().newInterfaceOp(toType->typeIndex, toType->typeAffiliateModule != currentModuleIndex,
                // toType->typeAffiliateModule);
                moduleContext->getIRBuilder().constructInterfaceImplOp({toType->typeAffiliateModule, toType->typeIndex},
                                                                       implIndex,
                                                                       rhs->typeAffiliateModule != currentModuleIndex,
                                                                       rhs->typeAffiliateModule);
            } catch (std::out_of_range &e) {
                panic(moduleContext->getIRBuilder().getCurrentDebugInfo().line,
                      moduleContext->getIRBuilder().getCurrentDebugInfo().column,
                      "Cannot cast type " + yoi::wstring2string((rhs->to_string())) + " to interface " + yoi::wstring2string((toType->to_string())) +
            ...
            if (result.found()) {
                ...
                moduleContext->getIRBuilder().invokeDanglingOp(
                    result.functionIndex, 2, result.function->returnType, true, toType->typeAffiliateModule);
            } else {
                panic(moduleContext->getIRBuilder().getCurrentDebugInfo().line,
                      moduleContext->getIRBuilder().getCurrentDebugInfo().column,
                      "Cannot cast type " + yoi::wstring2string((rhs->to_string())) + " to " + yoi::wstring2string((toType->to_string())) +
    ...
    bool visitor::canCastTo(const std::shared_ptr<IRValueType> &fromType, const std::shared_ptr<IRValueType> &toType) {
        ...
        if (*rhs == *toType) {
            return true;
                   ...
                   !rhs->isArrayType() && !toType->isArrayType() && (toType->type != IRValueType::valueType::stringObject) &&
                   (rhs->type != IRValueType::valueType::stringObject || toType->type == IRValueType::valueType::pointerObject)) {
            return true;
    ...
    yoi::indexT visitor::visit(yoi::dynCastExpression *dynCastExpression) {
        ...
        if (auto it = std::find(impls.begin(), impls.end(), std::make_tuple(toType->type, toType->typeAffiliateModule, toType->typeIndex));
            it != impls.end())
            moduleContext->getIRBuilder().dynCastOp(toType);
        else
            panic(dynCastExpression->getLine(),
                  dynCastExpression->getColumn(),
                  "Cannot cast type " + yoi::wstring2string((rhs->to_string())) + " to " + yoi::wstring2string((toType->to_string())) +
                      ": no implementation found.");
    ...
    yoi::indexT visitor::generateNullInterfaceImplementation(const std::shared_ptr<IRValueType> &structType) {
        ...
        try {
            return moduleContext->getCompilerContext()
                ->getImportedModule(structType->typeAffiliateModule)
                ->interfaceImplementationTable.getIndex(nullImplName);
        } catch (std::out_of_range &e) {
            moduleContext->getCompilerContext()
                ->getImportedModule(HOSHI_COMPILER_CTX_GLOB_ID_CONST)
                ->interfaceTable[0]
                ->implementations.emplace_back(structType->type, structType->typeAffiliateModule, structType->typeIndex);
            auto nullImpl = managedPtr(IRInterfaceImplementationDefinition{nullImplName,
            ...
            return moduleContext->getCompilerContext()
                ->getImportedModule(structType->typeAffiliateModule)
                ->interfaceImplementationTable.put_create(nullImplName, nullImpl);
        }
    }
    ...
    yoi::indexT visitor::visit(yoi::newExpression *newExpression) {
        ...
        if (newExpression->length)
            ...
            moduleContext->getIRBuilder().pushOp(IR::Opcode::push_integer,
                                                 IROperand{IROperand::operandType::integer, yoi::indexT{newExpression->args->get().size()}});
    ...
    visitor::OverloadResult visitor::resolveOverloadExtern(const yoi::wstr &baseName,
        ...
        if (targetedModule->functionTable.contains(lookupName)) {
            ...
            if (std::find(result.function->attrs.begin(), result.function->attrs.end(), IRFunctionDefinition::FunctionAttrs::Variadic) !=
        ...
        auto findVariadicMatch = [&](const yoi::wstr &funcKey, bool skipFirstParam = false) {
            ...
            if (std::find(func->attrs.begin(), func->attrs.end(), IRFunctionDefinition::FunctionAttrs::Variadic) != func->attrs.end()) {
                ...
                if (argTypes.size() != fixedParamCount + 1 ||
                    paramTypes.size() != fixedParamCount + 1 + skipFirstParam) // balance the variadic argument
                    return false;
            ...
            if (key.starts_with(prefix)) {
                if (findVariadicMatch(key,
                                      structContext != nullptr &&
                                          !targetedModule->functionTable[it]->hasAttribute(IRFunctionDefinition::FunctionAttrs::Static)))
                    return result;
            }
        }
        ...
        if (!structContext && targetedModule->funcTemplateAsts.contains(baseName)) {
            try {
                ...
                for (yoi::indexT i = 0; i < argTypes.size(); i++) {
                    if (i < templateArgs.size() && templateArgs[i].templateType->type == IRValueType::valueType::incompleteTemplateType) {
    ...
    bool visitor::handleInvocationExtern(const yoi::wstr &baseName,
                                         ...
                                         bool noThisCall,
                                         yoi::templateArg *templateArgs) {
        moduleContext->getIRBuilder().saveState();
        auto argTypes = evaluateArguments(args);
        OverloadResult overload;
        if (structContext && structContext->type == IRValueType::valueType::structObject) {
            auto structType =
                moduleContext->getCompilerContext()->getImportedModule(structContext->typeAffiliateModule)->structTable[structContext->typeIndex];
            ...
            if (!overload.found()) {
                ...
                if (structType->templateMethodDecls.contains(baseName) || structType->templateMethodDefs.contains(baseName)) {
                    ...
                    for (auto &arg : concreteMethodTemplateArgs) {
                        if (!arg) {
                            allDeduced = false;
                            break;
                        }
            ...
            auto interfaceType =
                moduleContext->getCompilerContext()->getImportedModule(structContext->typeAffiliateModule)->interfaceTable[structContext->typeIndex];
        ...
        if (overload.isVariadic) {
            ...
            if (variadicArgCount > 0) {
                ...
                moduleContext->getIRBuilder().newArrayOp(
                    overload.variadicElementType, {static_cast<yoi::indexT>(variadicArgCount)}, variadicArgCount);
            } else {
        ...
        if (structContext && structContext->type == IRValueType::valueType::structObject) {
            ...
            auto isStaticMethod =
                std::find(overload.function->attrs.begin(), overload.function->attrs.end(), IRFunctionDefinition::FunctionAttrs::Static) !=
                overload.function->attrs.end();
            bool usePureStaticLogic = noThisCall && isStaticMethod;

            if (usePureStaticLogic) {
                moduleContext->getIRBuilder().invokeOp(
                    externEntry.itemIndex, finalParamCount, overload.function->returnType, true, externEntry.affiliateModule);
            } else {
    ...
    template <typename T> yoi::indexT visitor::handleBinaryOperatorOverload(const yoi::wstr &overloadName, T *rhsAST) {
        auto lhs = moduleContext->getIRBuilder().getLhsFromTempVarStack();
        auto rhs = moduleContext->getIRBuilder().getRhsFromTempVarStack();
        bool isResolved = false;
        ...
        if (lhs->type == IRValueType::valueType::structObject) {
            auto resolved =
                resolveOverloadExtern(overloadName,
                                      {lhs, rhs},
                                      lhs->typeAffiliateModule,
                                      moduleContext->getCompilerContext()->getImportedModule(lhs->typeAffiliateModule)->structTable[lhs->typeIndex]);
            if (resolved.found()) {
                yoi_assert(resolved.isVariadic == false, 0, 0, "Binary operator overloading with variadic functions is not supported.");
                yoi_assert(std::find(resolved.function->attrs.begin(), resolved.function->attrs.end(), IRFunctionDefinition::FunctionAttrs::Static) !=
                               resolved.function->attrs.end(),
                           moduleContext->getIRBuilder().getCurrentDebugInfo().line,
                           moduleContext->getIRBuilder().getCurrentDebugInfo().column,
                           "Binary operator overloading with non-static functions is not supported.");
        ...
        if (!isResolved && rhs->type == IRValueType::valueType::structObject) {
            auto resolved =
                resolveOverloadExtern(overloadName,
                                      {lhs, rhs},
                                      rhs->typeAffiliateModule,
                                      moduleContext->getCompilerContext()->getImportedModule(rhs->typeAffiliateModule)->structTable[rhs->typeIndex]);
            if (resolved.found()) {
                yoi_assert(resolved.isVariadic == false,
                           moduleContext->getIRBuilder().getCurrentDebugInfo().line,
                           moduleContext->getIRBuilder().getCurrentDebugInfo().column,
        ...
        if (!isResolved && lhs->type == IRValueType::valueType::interfaceObject) {
            ...
            auto resolved = resolveOverloadInterface(
                baseName,
                {lhs, rhs},
                lhs->typeAffiliateModule,
                moduleContext->getCompilerContext()->getImportedModule(lhs->typeAffiliateModule)->interfaceTable[lhs->typeIndex]);
            ...
            if (resolved.found()) {
                ...
                moduleContext->getIRBuilder().invokeVirtualOp(
                    resolved.functionIndex, lhs->typeIndex, 1, resolved.function->returnType, true, lhs->typeAffiliateModule);
                isResolved = true;
            }
        }
        ...
        yoi_assert(isResolved,
                   moduleContext->getIRBuilder().getCurrentDebugInfo().line,
                   moduleContext->getIRBuilder().getCurrentDebugInfo().column,
                   "Binary operator overloading not found for " + yoi::wstring2string(overloadName));
    ...
    yoi::indexT visitor::handleUnaryOperatorOverload(const yoi::wstr &overloadName) {
        ...
        if (rhs->type == IRValueType::valueType::structObject) {
            ...
            if (resolved.found()) {
                yoi_assert(resolved.isVariadic == false,
                           moduleContext->getIRBuilder().getCurrentDebugInfo().line,
                           moduleContext->getIRBuilder().getCurrentDebugInfo().column,
                           "Unary operator overloading with variadic functions is not supported.");
                yoi_assert(std::find(resolved.function->attrs.begin(), resolved.function->attrs.end(), IRFunctionDefinition::FunctionAttrs::Static) !=
                               resolved.function->attrs.end(),
                           moduleContext->getIRBuilder().getCurrentDebugInfo().line,
                           moduleContext->getIRBuilder().getCurrentDebugInfo().column,
                           "Unary operator overloading with non-static functions is not supported.");
        ...
        if (rhs->type == IRValueType::valueType::interfaceObject) {
            ...
            auto methodIdx = moduleContext->getCompilerContext()
                                 ->getImportedModule(rhs->typeAffiliateModule)
                                 ->interfaceTable[rhs->typeIndex]
        ...
        yoi_assert(isResolved,
                   moduleContext->getIRBuilder().getCurrentDebugInfo().line,
                   moduleContext->getIRBuilder().getCurrentDebugInfo().column,
                   "Unary operator overloading not found for " + yoi::wstring2string(overloadName));
    ...
    yoi::indexT visitor::specializeInterfaceTemplate(const yoi::wstr &templateName,
        ...
        IRTemplateBuilder specializationContext;
        yoi_assert(concreteTemplateArgs.size() == interfaceAst->id->arg->get().size(),
                   0,
                   0,
                   "Template argument count mismatch for interface " + wstring2string(templateName));
        for (yoi::indexT i = 0; i < concreteTemplateArgs.size(); ++i) {
        ...
        auto interfaceIndex = irModule->interfaceTable.put_create(
            specializedName, specializedInterface); // since module context is still in foreign module, no need to change to targetedModule
    ...
    void visitor::specializeInterfaceImplementation(yoi::implStmt *implAst,
        ...
        yoi_assert(implAst->isImplForStmt(),
                   implAst->getLine(),
                   implAst->getColumn(),
                   "Expected 'impl for' AST node for interface implementation specialization.");
        ...
        yoi_assert(concreteInterfaceType->type == IRValueType::valueType::interfaceObject,
                   implAst->getLine(),
                   implAst->getColumn(),
        ...
        auto targetInterface =
            moduleContext->getCompilerContext()->getImportedModule(interfaceSrcPair.first)->interfaceTable[interfaceSrcPair.second];
        ...
        if (implAst->inner) {
            ...
            for (auto &methodNode : implAst->getInner().getInner()) {
                yoi_assert(!methodNode->isConstructor(),
                           methodNode->getLine(),
                           methodNode->getColumn(),
                           "Only methods are allowed in interface implementations.");
                ...
                auto baseMethodName = methodAst.getName().getId().get().strVal;
                ...
                moduleContext->getIRBuilder().yield();
                moduleContext->popIRBuilder();
                virtualMethodMap[baseMethodName + getFuncUniqueNameStr(specializedArgTypes, true)] = {
                    baseMethodName + uniq, managedPtr(IRValueType{IRValueType::valueType::virtualMethod, currentModuleIndex, funcIndex})};
            }
            for (auto &method : targetInterface->methodMap) {
                yoi_assert(virtualMethodMap.contains(method.first),
                           implAst->getLine(),
                           implAst->getColumn(),
                           "Interface method not found in implementation: " + wstring2string(method.first));
                builder.addVirtualMethod(virtualMethodMap[method.first].first, virtualMethodMap[method.first].second);
            }
    ...
    visitor::OverloadResult visitor::resolveOverloadInterface(const yoi::wstr &baseName,
                                                              const yoi::vec<std::shared_ptr<IRValueType>> &argTypes,
                                                              yoi::indexT targetModule,
    ...
    bool
    visitor::handleSubscript(yoi::vec<yoi::subscript *>::iterator &it, yoi::vec<yoi::subscript *>::iterator end, bool isStoreOp, bool isLastTerm) {
        auto objectOnStackType = moduleContext->getIRBuilder().getRhsFromTempVarStack();
        ...
        if (objectOnStackType->isArrayType() || objectOnStackType->isDynamicArrayType()) {
            ...
            while (it != end && (*it)->isSubscript()) {
                ...
                visit((*it)->expr);
                tryCastTo(moduleContext->getCompilerContext()->getUnsignedObjectType());
                yoi_assert(moduleContext->getIRBuilder().getRhsFromTempVarStack()->type == IRValueType::valueType::unsignedObject,
                           (*it)->getLine(),
                           (*it)->getColumn(),
                           "Array subscript index must be an integer or unsigned integer.");
                moduleContext->getIRBuilder().pushOp(IR::Opcode::push_unsigned, {IROperand::operandType::unsignedInt, strides[currentDim]});
                moduleContext->getIRBuilder().arithmeticOp(IR::Opcode::mul);
                moduleContext->getIRBuilder().arithmeticOp(IR::Opcode::add);
                it++;
                currentDim++;
            ...
            if (isLastTerm && isStoreOp) {
                ...
                OverloadResult overload;
                if (array->type == IRValueType::valueType::structObject)
                    overload = resolveOverloadExtern(
                        L"operator[]",
                        {value, array, index},
                        array->typeAffiliateModule,
                        moduleContext->getCompilerContext()->getImportedModule(array->typeAffiliateModule)->structTable[array->typeIndex]);

                yoi_assert(overload.found(), currentTerm->getLine(), currentTerm->getColumn(), "No matching overload found for operator[].");
                yoi_assert(
                    !overload.isVariadic, currentTerm->getLine(), currentTerm->getColumn(), "Variadic operator[] overloading is not supported.");
    ...
    IRValueType visitor::parseTypeSpec(yoi::funcTypeSpec *typeSpec) {
        ...
        return IRValueType{IRValueType::valueType::interfaceObject, HOSHI_COMPILER_CTX_GLOB_ID_CONST, createCallableInterface(argTypes, returnType)};
    }
    ...
    yoi::indexT visitor::createCallableInterface(const yoi::vec<std::shared_ptr<IRValueType>> &parameterTypes,
        ...
        if (moduleContext->getCompilerContext()
                ->getImportedModule(HOSHI_COMPILER_CTX_GLOB_ID_CONST)
                ->interfaceTable.contains(callableInterfaceName)) {
            return moduleContext->getCompilerContext()
                ->getImportedModule(HOSHI_COMPILER_CTX_GLOB_ID_CONST)
        ...
        builder.addMethod(L"operator()",
                          L"operator()" + getFuncUniqueNameStr(parameterTypes),
                          managedPtr(IRFunctionDefinition{L"operator()" + getFuncUniqueNameStr(parameterTypes), argTypes, returnType, {}, {}, {}}));
    ...
    yoi::indexT visitor::createLambdaUnnamedStruct(yoi::lambdaExpr *lambdaExpr) {
        ...
        moduleContext->pushIRBuilder(IRBuilder{moduleContext->getCompilerContext(), irModule, irModule->functionTable[callableFuncIndex]});
        ...
        moduleContext->pushIRBuilder(IRBuilder{moduleContext->getCompilerContext(), irModule, irModule->functionTable[constructorFuncIndex]});
        moduleContext->getIRBuilder().setDebugInfo({irModule->modulePath, lambdaExpr->getLine(), lambdaExpr->getColumn()});
        moduleContext->getIRBuilder().switchCodeBlock(moduleContext->getIRBuilder().createCodeBlock());
        for (yoi::indexT i = 0; i < lambdaExpr->captures.size(); ++i) {
            ...
            moduleContext->getIRBuilder().loadOp(IR::Opcode::load_local,
                                                 {IROperand::operandType::localVar, i + 1},
                                                 argTypes[i]); // take advantages of the argTypes which we haven't clear it yet.
            // store the parameter into the field
            // now we load the this pointer, and store the parameter into the field
            moduleContext->getIRBuilder().loadOp(
                IR::Opcode::load_local, {IROperand::operandType::localVar, IROperand::operandValue{static_cast<yoi::indexT>(0)}}, structType);
    ...
    std::pair<yoi::indexT, std::pair<yoi::indexT, yoi::indexT>> visitor::createCallableImplementationForLambda(
    ...
    bool visitor::checkMarcoSatisfaction(yoi::marcoDescriptor *desc) {
                ...
                case lexer::token::tokenKind::integer:
                    switch (rhs.kind) {
                        ...
                        case lexer::token::tokenKind::decimal:
                            return {lexer::token{0, 0, lexer::token::tokenKind::decimal, static_cast<double>(lhs.basicVal.vDeci)}, rhs};
                        default:
                            panic(lhs.line, lhs.col, "Cannot convert marco value to comparable type.");
                    }
                case lexer::token::tokenKind::decimal:
                    switch (rhs.kind) {
                        case lexer::token::tokenKind::integer:
                            return {lhs, lexer::token{0, 0, lexer::token::tokenKind::decimal, static_cast<double>(rhs.basicVal.vUint)}};
                        case lexer::token::tokenKind::decimal:
                            return {lhs, rhs};
                        default:
                            panic(lhs.line, lhs.col, "Cannot convert marco value to comparable type.");
                ...
                case lexer::token::tokenKind::string:
                    switch (rhs.kind) {
                        ...
                        case lexer::token::tokenKind::boolean:
                            return {lexer::token{0, 0, lexer::token::tokenKind::integer, static_cast<uint64_t>(lhs.strVal == rhs.strVal)}, rhs};
                        default:
                            panic(lhs.line, lhs.col, "Cannot convert marco value to comparable type.");
                    }
                case lexer::token::tokenKind::boolean:
        ...
        auto compare = [&](lexer::token &lhs, lexer::token &rhs, lexer::token::tokenKind op) {
            ...
            switch (op) {
                case lexer::token::tokenKind::equal:
                    return lhsTok.basicVal.vUint == rhsTok.basicVal.vUint && lhsTok.strVal == rhsTok.strVal;
                case lexer::token::tokenKind::notEqual:
                    return lhsTok.basicVal.vUint != rhsTok.basicVal.vUint || lhsTok.strVal != rhsTok.strVal;
                case lexer::token::tokenKind::greaterThan:
                    switch (lhsTok.kind) {
                        case lexer::token::tokenKind::integer:
                            return lhsTok.basicVal.vUint > rhsTok.basicVal.vUint;
                        case lexer::token::tokenKind::decimal:
                            return lhsTok.basicVal.vDeci > rhsTok.basicVal.vDeci;
                        default:
                            panic(lhs.line, lhs.col, "Cannot compare marco value.");
                    }
                case lexer::token::tokenKind::greaterEqual:
                    switch (lhsTok.kind) {
                        case lexer::token::tokenKind::integer:
        ...
        for (auto &i : desc->pairs) {
            ...
            switch (i->constraint.kind) {
                ...
                default: {
                    panic(desc->getLine(),
                          desc->getColumn(),
                          "Unsupported marco constraint kind: " + std::string{magic_enum::enum_name(i->constraint.kind)});
                    break;
                }
            }
            if (!currentSatisfied) {
    ...
    yoi::indexT visitor::visit(yoi::funcExpr *func) {
        ...
        auto targetedModule = moduleContext->getCompilerContext()->getImportedModule(targetModule == -1 ? currentModuleIndex : targetModule);
        enum class CreateStrategy { Plain, IncludeThis } strategy{CreateStrategy::Plain};
        yoi::indexT funcIndex = -1;
        ...
        if (it + 2 == func->name->getTerms().end()) {
            ...
            if (nameNode->hasTemplateArg() && targetedModule->structTemplateAsts.contains(nameNode->getId().node.strVal)) {
                ...
                auto specializedIndex = specializeStructTemplate(
                    nameNode->id->node.strVal, concreteTypes, targetedModule->templateImplAsts[nameNode->getId().node.strVal], targetModule);
            ...
            yoi_assert(targetedModule->functionOverloadIndexies.contains(baseName),
                       funcNameNode->getLine(),
                       funcNameNode->getColumn(),
                       "Undefined method: " + yoi::wstring2string(baseName));
            ...
            if (func->args) {
                ...
                yoi_assert(targetedModule->functionTable.contains(fullFuncName),
                           funcNameNode->getLine(),
                           funcNameNode->getColumn(),
                           "No matching function overload found with explicit param types: " + yoi::wstring2string(fullFuncName));
                yoi_assert(targetedModule->functionTable[fullFuncName]->hasAttribute(IRFunctionDefinition::FunctionAttrs::Static),
                           funcNameNode->getLine(),
                           funcNameNode->getColumn(),
                           "Cannot call non-static method: " + yoi::wstring2string(fullFuncName));
                ...
                yoi_assert(targetedModule->functionOverloadIndexies[baseName].size() == 1,
                           funcNameNode->getLine(),
                           funcNameNode->getColumn(),
                           "Inplicit specification on multiple overloads of method: " + yoi::wstring2string(baseName));
                auto candidateIndex = targetedModule->functionOverloadIndexies[baseName][0];
                yoi_assert(targetedModule->functionTable[candidateIndex]->hasAttribute(IRFunctionDefinition::FunctionAttrs::Static),
                           funcNameNode->getLine(),
                           funcNameNode->getColumn(),
                           "Cannot call non-static method: " + yoi::wstring2string(baseName));
            ...
            yoi_assert(targetedModule->functionOverloadIndexies.contains(funcNameNode->getId().node.strVal),
                       funcNameNode->getLine(),
                       funcNameNode->getColumn(),
                       "Undefined function: " + yoi::wstring2string(funcNameNode->getId().node.strVal));
            ...
            if (func->args) {
                ...
                // no params, check whether the function table contains only one
                yoi_assert(targetedModule->functionOverloadIndexies[funcNameNode->getId().node.strVal].size() == 1,
                           funcNameNode->getLine(),
                           funcNameNode->getColumn(),
                           "Inplicit specification on multiple overloads of function: " + yoi::wstring2string(funcNameNode->getId().node.strVal));
    ...
    std::pair<yoi::indexT, std::pair<yoi::indexT, yoi::indexT>> visitor::createCallableImplementationForFunction(
        ...
        if (targetedModule->structTable.contains(uniqueName)) {
            auto interfaceImplName = getInterfaceImplName(
                callableInterface,
                managedPtr(IRValueType{IRValueType::valueType::structObject, moduleIndex, targetedModule->structTable.getIndex(uniqueName)}));
        ...
        auto interfaceImplIndex = targetedModule->interfaceImplementationTable.put_create(
            ...
            IRInterfaceImplementationDefinition::Builder()
                ...
                .addVirtualMethod(L"operator()" + getFuncUniqueNameStr(func->argumentTypes),
