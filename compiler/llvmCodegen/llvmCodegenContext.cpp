...
namespace yoi {
    ...
    LLVMCodegen::LLVMCodegen(std::shared_ptr<compilerContext> compilerCtx, const std::shared_ptr<IRModule> &yoiModule)
        ...
        codegenObjectCache.setCompilerCtx(this->compilerCtx);
    ...
    void LLVMCodegen::declareRuntimeFunctions(LLVMModuleContext &llvmModCtx) {
        ...
        llvm::Type* i8PtrTy = llvm::PointerType::get(*llvmModCtx.TheContext, 0);
        llvm::Type* sizeTy = llvmModCtx.Builder->getInt64Ty();

        llvm::FunctionType *mallocFuncType = llvm::FunctionType::get(llvm::PointerType::get(*llvmModCtx.TheContext, 0), {sizeTy, sizeTy}, false);
    ...
    void LLVMCodegen::generateBasicTypeDeclarations(LLVMModuleContext &llvmModCtx) {
        ...
        std::vector<std::pair<std::shared_ptr<IRValueType>, llvm::Type*>> basicTypes = {
            ...
            {compilerCtx->getStrObjectType(), llvm::PointerType::get(*llvmModCtx.TheContext, 0)},
    ...
    void LLVMCodegen::generateBasicTypeImplementations(LLVMModuleContext &llvmModCtx) {
        ...
        for (const auto& pair : basicTypes) {
            ...
            if (compilerCtx->getBuildConfig()->buildMode == IRBuildConfig::BuildMode::debug) {
                ...
                auto* debugStrPtr = llvmModCtx.Builder->CreateBitCast(debugStrGlobal, llvm::PointerType::get(*llvmModCtx.TheContext, 0));
                llvmModCtx.Builder->CreateCall(llvmModCtx.runtimeFunctions.at(L"runtime_debug_print"), debugStrPtr);
                // address
                auto* castedPtr = llvmModCtx.Builder->CreateBitCast(thisPtr, llvm::PointerType::get(*llvmModCtx.TheContext, 0));
    ...
    void LLVMCodegen::generateGlobalInitializers(LLVMModuleContext &llvmModCtx) {
        for (auto& globalPair : yoiModule->globalVariables) {
            ...
            // null initializer
            auto initializer = llvm::Constant::getNullValue(llvm::PointerType::get(*llvmModCtx.TheContext, 0));
    ...
    void LLVMCodegen::generateStructDeclarations(LLVMModuleContext &llvmModCtx) {
        ...
        for (auto& interfaceDefPair : yoiModule->interfaceTable) {
            ...
            for (const auto& methodPair : interfaceDef->methodMap) {
                ...
                virtualArgTypes.push_back(llvm::PointerType::get(*llvmModCtx.TheContext, 0)); // 'this' is always i8*
                ...
                memberTypes.push_back(llvm::PointerType::get(*llvmModCtx.TheContext, 0));
            }
            memberTypes.push_back(llvm::PointerType::get(*llvmModCtx.TheContext, 0)); // weak_slots_head
    ...
    void LLVMCodegen::generateInterfaceObjectGCFunctionDeclarations(LLVMModuleContext &llvmModCtx) {
        ...
        for (const auto& interfaceDefPair : yoiModule->interfaceTable) {
            ...
            auto* llvmInterfaceType = llvmModCtx.structTypeMap.at(key);
            auto* llvmInterfacePtrType = llvm::PointerType::get(*llvmModCtx.TheContext, 0);
            ...
            auto* gcFuncPtrTypeForDispatch = llvm::PointerType::get(*llvmModCtx.TheContext, 0);
    ...
    void LLVMCodegen::generateInterfaceObjectGCFunctionImplementations(LLVMModuleContext &llvmModCtx) {
        for (const auto& interfaceDefPair : yoiModule->interfaceTable) {
            ...
            // Nullify all weak slots pointing to this interface before freeing
            auto weakSlotsHeadIdx = llvmInterfaceType->getNumElements() - 1;
            auto* weakSlotsHeadPtr = llvmModCtx.Builder->CreateStructGEP(llvmInterfaceType, thisPtr, weakSlotsHeadIdx, "weak_slots_head_ptr");
            llvmModCtx.Builder->CreateCall(llvmModCtx.runtimeFunctions.at(L"runtime_weak_slot_nullify_all"), {weakSlotsHeadPtr});
        ...
        switch(instr.opcode) {
            ...
            case IR::Opcode::load_member: {
                ...
                if (memberYoiType->hasAttribute(IRValueType::ValueAttr::WeakRef)) {
                    ...
                    auto* phi = llvmModCtx.Builder->CreatePHI(i8PtrTy, 2, "weak_load_phi");
                    phi->addIncoming(targetPtr, retainTargetBB);
                    phi->addIncoming(llvm::ConstantPointerNull::get(i8PtrTy), pushNullBB);
                    llvmModCtx.valueStackPhi.push_back({phi, nullType});
            ...
            case IR::Opcode::new_datastruct: {
                ...
                llvmModCtx.valueStackPhi.push_back({allocCall, yoiType});
                break;
            }
            case IR::Opcode::initialize_field: {
                yoi::vec<StackValue> values(instr.operands[0].value.symbolIndex);
                for (yoi::indexT i = instr.operands[0].value.symbolIndex; i > 0; i--) {
                    values[i - 1] = llvmModCtx.valueStackPhi.back();
                    llvmModCtx.valueStackPhi.pop_back();
                }
                ...
                for (yoi::indexT i = 0; i < instr.operands[0].value.symbolIndex; i++) {
                    ...
                    auto fieldType = dataRegionType->getStructElementType(i);
                    if (fieldType->isStructTy()) {
                        // create MemCpy
                        auto size = llvmModCtx.TheModule->getDataLayout().getTypeAllocSize(fieldType);
                        auto *sizeVal = llvm::ConstantInt::get(llvmModCtx.Builder->getInt64Ty(), size);
                        auto *memCpy = llvmModCtx.Builder->CreateMemCpy(fieldPtr, llvm::MaybeAlign(8), value, llvm::MaybeAlign(8), sizeVal);
                    } else {
            ...
            case IR::Opcode::store_field: {
                auto destValue = llvmModCtx.valueStackPhi.back();
                llvmModCtx.valueStackPhi.pop_back();

                auto srcValue = llvmModCtx.valueStackPhi.back();
                llvmModCtx.valueStackPhi.pop_back();

                auto currentType = srcValue.yoiType;
                auto val = unboxValue(llvmModCtx, srcValue.llvmValue, srcValue.yoiType);
                auto llvmType = yoiTypeToLLVMType(llvmModCtx, currentType, true);

                auto destType = destValue.yoiType;
                auto destVal = unboxValue(llvmModCtx, destValue.llvmValue, destValue.yoiType);
                llvm::Value *fieldPtr = nullptr;

                for (auto &operand : instr.operands) {
                    auto def = yoiModule->dataStructTable[destType->typeIndex];
                    auto nextType = def->fieldTypes[operand.value.symbolIndex];
                    auto llvmType = llvmModCtx.dataStructDataRegionMap.at(destType->typeIndex);
                    fieldPtr = llvmModCtx.Builder->CreateStructGEP(llvmType, destVal, operand.value.symbolIndex, "field_ptr");
                    destVal = fieldPtr;
                    destType = nextType;
                }
                // check if assigning to datastruct
                if (destType->type == IRValueType::valueType::datastructObject) {
                    // create MemCpy
                    auto size = llvmModCtx.TheModule->getDataLayout().getTypeAllocSize(llvmType);
                    auto *sizeVal = llvm::ConstantInt::get(llvmModCtx.Builder->getInt64Ty(), size);
                    auto *memCpy = llvmModCtx.Builder->CreateMemCpy(fieldPtr, llvm::MaybeAlign(8), val, llvm::MaybeAlign(8), sizeVal);
                } else {
            ...
            case IR::Opcode::load_field: {
                auto top = llvmModCtx.valueStackPhi.back();
                llvmModCtx.valueStackPhi.pop_back();
                auto currentType = top.yoiType;
                auto val = unboxValue(llvmModCtx, top.llvmValue, top.yoiType);
                for (auto &operand : instr.operands) {
                    auto def = yoiModule->dataStructTable[currentType->typeIndex];
                    auto nextType = def->fieldTypes[operand.value.symbolIndex];
                    auto llvmType = llvmModCtx.dataStructDataRegionMap.at(currentType->typeIndex);
                    val = llvmModCtx.Builder->CreateStructGEP(llvmType, val, operand.value.symbolIndex, "field_ptr");
                    currentType = nextType;
                }
                if (currentType->isArrayType()) {
                    auto arrayVal = createArrayObject(llvmModCtx, managedPtr(currentType->getElementType()), {});
                    auto llvmType = getArrayLLVMType(llvmModCtx, currentType);
                    ...
                    val = arrayVal;
                    currentType = managedPtr(*currentType);
                } else if (currentType->isBasicType()) {
                    val = llvmModCtx.Builder->CreateLoad(yoiTypeToLLVMType(llvmModCtx, currentType, true), val, "field_val");
                    currentType = managedPtr(*currentType);
                    currentType->addAttribute(IRValueType::ValueAttr::Raw);
                } else if (currentType->type == IRValueType::valueType::datastructObject) {
                    // otherwise this is a data struct perform nothing
                    currentType = managedPtr(*currentType);
                    currentType->addAttribute(IRValueType::ValueAttr::Raw);
                } else {
                    panic(instr.debugInfo.line, instr.debugInfo.column, "unreachable");
                }
                llvmModCtx.valueStackPhi.push_back({val, currentType});
                break;
            }
            ...
            case IR::Opcode::invoke_virtual: {
                ...
                auto* bitcastedPointer = llvmModCtx.Builder->CreateBitCast(concreteThisPtrRaw, llvm::PointerType::get(*llvmModCtx.TheContext, 0), "casted_this");
                ...
                if (interfaceShellVal.yoiType->metadata.hasMetadata(L"regressed_interface_impl")) {
                    ...
                    std::vector<llvm::Type*> virtualArgTypes;
                    virtualArgTypes.push_back(llvm::PointerType::get(*llvmModCtx.TheContext, 0));
                    ...
                    auto* virtualFuncPtrType = llvm::PointerType::get(*llvmModCtx.TheContext, 0);
            ...
            case IR::Opcode::dyn_cast_any: {
                ...
                if (interfaceRhs.yoiType->metadata.hasMetadata(L"regressed_interface_impl")) {
                    ...
                    if (implDef->implStructIndex == structTypeKey) {
                        ...
                        auto nullValue = llvm::ConstantPointerNull::get(llvm::PointerType::get(*llvmModCtx.TheContext, 0));
                ...
                auto *finalValue = llvmModCtx.Builder->CreatePHI(llvm::PointerType::get(*llvmModCtx.TheContext, 0), 2, "final_value");
            ...
            case IR::Opcode::pointer_cast: {
                auto rhs = llvmModCtx.valueStackPhi.back(); llvmModCtx.valueStackPhi.pop_back();
                auto value = llvmModCtx.Builder->CreateBitCast(rhs.llvmValue, llvm::PointerType::get(*llvmModCtx.TheContext, 0), "pointer_cast");
    ...
    llvm::Type* LLVMCodegen::yoiTypeToLLVMType(LLVMModuleContext &llvmModCtx, const std::shared_ptr<IRValueType>& type, bool enforceForeignType) {
        if (enforceForeignType) {
            if (type->isArrayType()) {
                ...
                return llvm::ArrayType::get(yoiTypeToLLVMType(llvmModCtx, managedPtr(type->getElementType()), true), count);
            }
            ...
            if (llvmModCtx.structTypeMap.count(key)) {
                if (type->isArrayType() || type->isDynamicArrayType()) {
                    getArrayLLVMType(llvmModCtx, type);
                    return llvm::PointerType::get(*llvmModCtx.TheContext, 0);
                } else {
                    return llvm::PointerType::get(*llvmModCtx.TheContext, 0);
                }
            }
    ...
    llvm::Value* LLVMCodegen::createBasicObject(LLVMModuleContext &llvmModCtx, const std::shared_ptr<IRValueType>& yoiType, llvm::Value* rawValue) {
        ...
        auto* newObjPtr = llvmModCtx.Builder->CreateBitCast(allocCall, llvm::PointerType::get(*llvmModCtx.TheContext, 0), "new_obj_ptr");
    ...
    void LLVMCodegen::generateMainFunction(LLVMModuleContext &llvmModCtx) {
        if (compilerCtx->getBuildConfig()->buildType == IRBuildConfig::BuildType::executable) {
            yoi::vec<llvm::Type*> argTypes = {
                ...
                llvm::PointerType::get(*llvmModCtx.TheContext, 0)
            };
        ...
        switch (foreignType->type) {
            ...
            case IRValueType::valueType::pointer: {
                if (convertToForeign) {
                    ...
                    // bit cast void*
                    auto *voidPtr = llvmModCtx.Builder->CreateIntToPtr(val, llvm::PointerType::get(*llvmModCtx.TheContext, 0), "void_ptr");
                    return voidPtr;
                } else {
                    // bitcast to i64
                    auto *voidPtr = llvmModCtx.Builder->CreateBitCast(val, llvm::PointerType::get(*llvmModCtx.TheContext, 0), "void_ptr");
        ...
        if (enforceForeignType) {
            ...
            switch (type->type) {
                ...
                case IRValueType::valueType::stringObject:
                    baseType = llvm::PointerType::get(*llvmModCtx.TheContext, 0);
                    break;
                case IRValueType::valueType::structObject:
                case IRValueType::valueType::interfaceObject:
                    baseType = llvm::PointerType::get(*llvmModCtx.TheContext, 0); // only this is a object
                    break;
                default:
    ...
    llvm::Value *LLVMCodegen::createArrayObject(LLVMModuleContext &llvmModCtx, const std::shared_ptr<IRValueType> &type,
        ...
        for (auto &i : elements) {
            ...
            if (type->isBasicType()) {
                ...
                auto arrayPointer = llvmModCtx.Builder->CreateGEP(llvm::PointerType::get(*llvmModCtx.TheContext, 0), arrayBasePointer, {llvm::ConstantInt::get(llvm::Type::getInt64Ty(*llvmModCtx.TheContext), index)}, "array_element_ptr");
    ...
    llvm::DIType *LLVMCodegen::getDIType(LLVMModuleContext &llvmModCtx, const std::shared_ptr<IRValueType> &type) {
        ...
        if (type->isArrayType() || type->isDynamicArrayType()) {
            ...
            if (type->isArrayType()) {
                for (yoi::indexT i : type->dimensions) {
                    size *= i;
                    dimensions.push_back(llvmModCtx.DBuilder->getOrCreateSubrange(0, static_cast<int64_t>(i)));
                }
            } else {
            ...
            if (llvmModCtx.arrayDataRegionDITypeMap.count(arrayKey) && type->hasAttribute(IRValueType::ValueAttr::Raw)) {
                return llvmModCtx.arrayDataRegionDITypeMap[arrayKey];
            ...
            auto *llvmElementRawType = yoiTypeToLLVMType(llvmModCtx, managedPtr(type->getElementType()), type->hasAttribute(IRValueType::ValueAttr::Raw));
            auto arraySizeInBits = size * llvmModCtx.TheModule->getDataLayout().getTypeSizeInBits(llvmElementRawType);
            auto* diArray = llvmModCtx.DBuilder->createArrayType(
                arraySizeInBits, llvmModCtx.TheModule->getDataLayout().getABITypeAlign(llvmElementRawType).value() * 8, elementDIType, {llvmModCtx.DBuilder->getOrCreateArray(dimensions)});

            llvmModCtx.arrayDataRegionDITypeMap[arrayKey] = diArray;
    ...
    void LLVMCodegen::generateRTTIDeclaration(LLVMModuleContext &llvmModCtx) {
        ...
        llvmModCtx.RTTIEntryType = llvm::StructType::get(*llvmModCtx.TheContext, {
            llvm::Type::getInt64Ty(*llvmModCtx.TheContext), // type id
            llvm::PointerType::get(*llvmModCtx.TheContext, 0), // type name
    ...
    void LLVMCodegen::generateArrayGCFunctionDeclarations(LLVMModuleContext &llvmModCtx, const std::shared_ptr<IRValueType> &type, llvm::StructType *structType, llvm::Type *baseType) {
        ...
        if (auto it = llvmModCtx.functionMap.find(yoi::string2wstring(incFuncName)) == llvmModCtx.functionMap.end()) {
            auto gcIncFuncType = llvm::FunctionType::get(
                llvm::Type::getVoidTy(*llvmModCtx.TheContext), {llvm::PointerType::get(*llvmModCtx.TheContext, 0)}, false);
            auto gcIncFunc =
    ...
    void LLVMCodegen::generateArrayGCFunctionImplementations(LLVMModuleContext &llvmModCtx, const std::shared_ptr<IRValueType> &type,
        ...
        }
        {
            ...
            if (type->type == IRValueType::valueType::structObject ||
                ...
                auto elementPointer =
                    llvmModCtx.Builder->CreateGEP(llvm::PointerType::get(*llvmModCtx.TheContext, 0),
                                       arrayPointer,
                                       {llvmModCtx.Builder->CreateLoad(llvm::Type::getInt64Ty(*llvmModCtx.TheContext), currentIndex)},
                                       "element_ptr");
                auto elementPointerVal =
                    llvmModCtx.Builder->CreateLoad(llvm::PointerType::get(*llvmModCtx.TheContext, 0),
                                        elementPointer,
    ...
    yoi::vec<yoi::wstr> LLVMCodegen::generate() {
        ...
        std::set<yoi::indexT> dirtyModules;
        auto &allModules = compilerCtx->getCompiledModules();

        // Identify initially dirty modules (disk changes)
        for (auto const &[id, module] : allModules) {
            if (module->modulePath == L"builtin") continue;
            yoi::indexT last_write_time = std::filesystem::last_write_time(module->modulePath).time_since_epoch().count();
            if (last_write_time != codegenObjectCache.get_entry(module->modulePath).getLastModification()) {
                dirtyModules.insert(id);
            }
        }
        ...
        for (auto &pair : allModules) {
            if (pair.second->modulePath == L"builtin") {
                continue;
            }
            auto id = pair.first;
            auto module = pair.second;
            codegenTaskDispatcher.dispatch([this, id, module, &dirtyModules]() {
                set_current_file_path(module->modulePath);
                if (dirtyModules.find(id) == dirtyModules.end()) {
                    warning(0, 0, "llvmCodegen: skipping module " + wstring2string(module->modulePath), "MODULE_NOT_MODIFIED");
                    return;
                }

                auto cache_entry = codegenObjectCache.get_entry(module->modulePath);
                
                generateImplementations(*llvmModuleContext[module->modulePath]);
                llvmModuleContext[module->modulePath]->DBuilder->finalize();
                generateTargetObjectCode(*llvmModuleContext[module->modulePath], cache_entry.getObjectFilename());

                yoi::indexT last_write_time = std::filesystem::last_write_time(module->modulePath).time_since_epoch().count();
                codegenObjectCache.update_last_modification(module->modulePath, last_write_time);
            });
        }
        ...
        for (auto &module : compilerCtx->getCompiledModules()) {
            ...
            dumpIR(module.second->modulePath, (cache_path / (std::to_string(module.second->identifier) + ".ll")).string());
        }
    ...
    llvm::StructType *LLVMCodegen::findStructLLVMType(const std::shared_ptr<IRValueType> &targetYoiType) {
        auto targetKey = std::make_tuple(targetYoiType->type, targetYoiType->typeAffiliateModule, targetYoiType->typeIndex);
        for (auto &[_, ctx] : llvmModuleContext) {
            if (ctx->structTypeMap.count(targetKey))
                return ctx->structTypeMap.at(targetKey);
        }
        return nullptr;
    }
    ...
    llvm::Value *LLVMCodegen::emitWeakSlotAlloc(LLVMModuleContext &llvmModCtx, llvm::Value *targetPtr, const std::shared_ptr<IRValueType> &targetYoiType) {
        auto* i8PtrTy = llvm::PointerType::get(*llvmModCtx.TheContext, 0);
        auto* targetLLVMType = findStructLLVMType(targetYoiType);
        if (targetLLVMType) {
            auto targetWeakSlotsIdx = targetLLVMType->getNumElements() - 1;
