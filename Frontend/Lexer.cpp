...
namespace Hoshi {
    ...
    Lexer::Token Lexer::Scan() {
        ...
        if (IsAlpha(String[Position])) {
            ...
            TempStr = String.substr(Start, Position - Start);
            NextCharacter(); // Skip
            LastToken = {TokenKind::Character, unescape_string(TempStr), Line, Column};
        }
            /* Expression symbols */
            /* Starts with :*/
        else if (String[Position] == L':') {
            ...
            if (String[Position] == L'=') {
                ...
                LastToken = {TokenKind::Slash, (std::wstring){std::filesystem::path::preferred_separator}, Line, Column};
            }
        }
            /* Starts with % */
        else if (String[Position] == L'%') {
    ...
    Lexer::Token::Token(Lexer::TokenKind Kind, XString Value, XIndexType Line, XIndexType Column) : Kind(Kind), Line(Line),
                                                                                                Column(Column),
                                                                                                Value(std::move(
                                                                                                        Value)) {}
