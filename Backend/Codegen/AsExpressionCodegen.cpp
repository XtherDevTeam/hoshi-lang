...
namespace Hoshi {
    ...
    Operand AsExpressionCodegen::Visit(AsExpressionNode &Node, IRProgram::Builder &Program,
        ...
        Operand LastResult = BooleanExpressionCodegen::INSTANCE.Visit(*Node.GetOperands(0), Program,
                                                                     Block); //The result of last expression ir
        for (int i = 0; i < Node.GetOperators().size(); i++) {
            Lexer::Token OperatorNode = Node.GetOperators(i);
            Operand ThisOperand = BooleanExpressionCodegen::INSTANCE.Visit(*Node.GetOperands(i + 1), Program,
