...
namespace Hoshi {
    ...
    Operand LogicComparingExpressionCodegen::Visit(LogicComparingExpressionNode &Node, IRProgram::Builder &Program,
        ...
        Operand LastResult = BinaryShiftExpressionCodegen::INSTANCE.Visit(*Node.GetOperands(0), Program,
                                                                          Block); //The result of last expression ir
        for (int i = 0; i < Node.GetOperators().size(); i++) {
            Lexer::Token OperatorNode = Node.GetOperators(i);
            Operand ThisOperand = BinaryShiftExpressionCodegen::INSTANCE.Visit(*Node.GetOperands(i + 1), Program,
