...
namespace Hoshi {
    ...
    Operand MuliplicationExpressionCodegen::Visit(MuliplicationExpressionNode &Node, IRProgram::Builder &Program) {
        ...
        Operand LastResult = UniqueExpressionCodegen::INSTANCE.Visit(*Node.GetOperands(0), Program); //The result of last expression ir
        for (int i = 0 ; i < Node.GetOperators().size() ; i ++) {
            Lexer::Token OperatorNode = Node.GetOperators(i);
            Operand ThisOperand = UniqueExpressionCodegen::INSTANCE.Visit(*Node.GetOperands(i + 1), Program); //the operand of this ir
