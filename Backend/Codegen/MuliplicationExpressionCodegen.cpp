...
namespace Hoshi {
     ...
     */
    Operand MuliplicationExpressionCodegen::Visit(MuliplicationExpressionNode &Node, IRProgram::Builder &Program) {
        IRBlock::Builder &Block = *Program.GetContext().CurrentBlock;
        Operand LastResult = UniqueExpressionCodegen::INSTANCE.Visit(*Node.GetOperands(0),
        ...
        for (int i = 0; i < Node.GetOperators().size(); i++) {
            ...
            Operand ThisOperand = UniqueExpressionCodegen::INSTANCE.Visit(*Node.GetOperands(i + 1),
            ...
            switch (OperatorNode.Kind) { //Choose Opcode
                ...
                case Lexer::TokenKind::PercentSign:
                    ...
                    break; // '%'
                default:
                    throw CompilerError(Node.Line, Node.Column, L"Invalid Muliplication Expression Operator!");
            }
