...
namespace Hoshi {
    ...
    Operand BinaryExpressionCodegen::Visit(BinaryExpressionNode &Node, IRProgram::Builder &Program) {
        ...
        Operand LastResult = LogicEqualExpressionCodegen::INSTANCE.Visit(*Node.GetOperands(0), Program); //The result of last expression ir
        for (int i = 0; i < Node.GetOperators().size(); i++) {
            Lexer::Token OperatorNode = Node.GetOperators(i);
            Operand ThisOperand = LogicEqualExpressionCodegen::INSTANCE.Visit(*Node.GetOperands(i + 1), Program); //the operand of this ir
