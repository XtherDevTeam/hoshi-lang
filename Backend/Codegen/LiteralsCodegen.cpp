    ...
    Operand LiteralsCodegen::Visit(LiteralsNode &Node, IRProgram::Builder &Program) {
        switch (Node.GetLiterals().Kind) {
            case Lexer::TokenKind::Integer:
                return Operand(OperandType::Integer, std::move(Node.GetLiterals().Value));
            case Lexer::TokenKind::Decimal:
                return Operand(OperandType::Decimal, std::move(Node.GetLiterals().Value));
            case Lexer::TokenKind::Boolean:
                return Operand(OperandType::Boolean, std::move(Node.GetLiterals().Value));
            case Lexer::TokenKind::Character:
                return Operand(OperandType::Character, std::move(Node.GetLiterals().Value));
            case Lexer::TokenKind::String:
                return Operand(OperandType::ReadonlyStringLiteral, std::move(Node.GetLiterals().Value));
        }
