...
namespace Hoshi {
     ...
     */
    IR::IR(const Opcode Code, const Operand SourceOperandA, const Operand SourceOperandB,
           const Operand DestinationOperand)
            : Code(Code), SourceOperandA(SourceOperandA), SourceOperandB(SourceOperandB),
     ...
     */
    Operand IR::GetSourceOperandA() const {
     ...
     */
    Operand IR::GetSourceOperandB() const {
     ...
     */
    Operand IR::GetDestinationOperand() const {
    ...
    XString ToString(IR ir) {
        XString str;
        if (ir.DestinationOperand.GetType() == OperandType::Identifier &&
            !ir.DestinationOperand.GetValue().empty()) { //Destination exists
            str += ir.DestinationOperand.GetValue();
            str += L" = ";
        }
        str += ToString(ir.Code);
        if (ir.SourceOperandA.GetType() != OperandType::Unknown &&
            !ir.SourceOperandA.GetValue().empty()) { //Source exists
            str += L" ";
            str += ir.SourceOperandA.GetValue();
            if (ir.SourceOperandB.GetType() != OperandType::Unknown &&
                !ir.SourceOperandB.GetValue().empty()) { //Source exists
                str += L", ";
